...
function investmentCard(inv, onView, onEdit, onDelete) {
  ...
  card.innerHTML = `
    ...
    <div class="detail-meta-grid" style="margin-top:var(--sp-2)">
      <div class="detail-meta-item">
        <span class="detail-meta-item__label">${icons.cash}Capital investi</span>
        <span class="detail-meta-item__value mono">${fmtAmount(inv.initialAmount)}</span>
      </div>
      <div class="detail-meta-item">
        <span class="detail-meta-item__label">${icons.wallet}Valeur actuelle</span>
        <span class="detail-meta-item__value mono">${fmtAmount(inv.currentValue)}</span>
      </div>
      <div class="detail-meta-item">
        <span class="detail-meta-item__label">${icons.bolt}Gain/Perte</span>
        <span class="detail-meta-item__value mono" style="color:${performanceColor(gain)}">${gain >= 0 ? '+' : ''}${fmtAmount(gain)}</span>
      </div>
      <div class="detail-meta-item">
        <span class="detail-meta-item__label">${icons.compass}Performance</span>
...
function investmentDetailView(inv, onBack) {
  ...
  wrap.innerHTML = `
    ...
    <section class="card" style="margin-top:var(--sp-3)">
      ...
      <div class="detail-meta-grid" style="margin-top:var(--sp-2)">
        <div class="detail-meta-item">
          <span class="detail-meta-item__label">${icons.briefcase}Type</span>
          <span class="detail-meta-item__value">${store.getInvestmentTypeInfo(inv.type).label}</span>
        </div>
        <div class="detail-meta-item">
          <span class="detail-meta-item__label">${icons.calendar}Date</span>
        ...
        ${account ? `
        <div class="detail-meta-item">
          <span class="detail-meta-item__label">${icons.bank}Compte associé</span>
    ...
    <section class="card" style="margin-top:var(--sp-3)">
      <div class="detail-section-title">${icons.compass}Évolution de la valeur</div>
...
export function InvestmentsScreen() {
  ...
  summaryCard.innerHTML = `
    ...
    <div class="detail-meta-grid" style="margin-top:var(--sp-3)">
      <div class="detail-meta-item">
        <span class="detail-meta-item__label">${icons.cash}Capital total</span>
        <span class="detail-meta-item__value mono">${fmtAmount(summary.totalInitial)}</span>
      </div>
      <div class="detail-meta-item">
        <span class="detail-meta-item__label">${icons.bolt}Gain/Perte total</span>
      ...
      <div class="detail-meta-item">
        <span class="detail-meta-item__label">${icons.compass}Performance globale</span>
    ...
    formCard.innerHTML = `
      ...
      <form id="inv-form" novalidate>
        <div class="form-group">
          <label class="form-label" for="f-inv-name">${icons.edit}Nom</label>
        ...
        <div class="form-group" style="margin-top:var(--sp-3)">
          <label class="form-label" for="f-inv-type">${icons.briefcase}Type</label>
        ...
        <div class="form-group" style="margin-top:var(--sp-3)">
          <label class="form-label" for="f-inv-initial">${icons.cash}Capital investi (MGA)</label>
        ...
        <div class="form-group" style="margin-top:var(--sp-3)">
          <label class="form-label" for="f-inv-current">${icons.wallet}Valeur actuelle (MGA)</label>
        ...
        <div class="form-group" style="margin-top:var(--sp-3)">
          <label class="form-label" for="f-inv-date">${icons.calendar}Date</label>
          <input class="form-input" type="date" id="f-inv-date" name="date" value="${editingInv?.date ?? new Date().toISOString().slice(0, 10)}" />
        </div>
        ${accounts.length > 0 ? `
        <div class="form-group" style="margin-top:var(--sp-3)">
          <label class="form-label" for="f-inv-account">${icons.bank}Compte associé (optionnel)</label>
        ...
        <div class="form-group" style="margin-top:var(--sp-3)">
          <label class="form-label" for="f-inv-notes">${icons.notes}Notes</label>
