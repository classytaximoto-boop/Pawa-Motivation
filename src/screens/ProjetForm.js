...
export function ProjetForm(params = {}) {
  const editingId = params.id ?? null;
  const existing = editingId ? store.getProject(editingId) : null;
  ...
  let selectedCategory = existing?.category ?? 'business';
  ...
  header.innerHTML = `
    ...
    <h1 style="font-size:var(--fs-xl)">${existing ? 'Modifier le projet' : 'Nouveau projet'}</h1>
  `;
  header.querySelector('.back-btn').addEventListener('click', () => {
    router.navigate(existing ? `/projets/${existing.id}` : '/projets');
  });
  ...
  screen.innerHTML = `
    <form id="project-form" novalidate>
      <div class="form-group">
        ...
        <input class="form-input" id="f-name" name="name" placeholder="Ex. Lancer le site vitrine" value="${existing?.name ?? ''}" required />
      </div>
      ...
      <div class="form-group" style="margin-top: var(--sp-4)">
        ...
        <textarea class="form-textarea" id="f-desc" name="description" placeholder="Décris ton projet...">${existing?.description ?? ''}</textarea>
      </div>
  ...
  screen.querySelector('#project-form').addEventListener('submit', (e) => {
    e.preventDefault();
    ...
    if (existing) {
      ...
      router.navigate(`/projets/${existing.id}`);
    } else {
      const id = store.createProject(fields);
      router.navigate(`/projets/${id}`);
    }
  });
