...
const defaultState = {
  ...
  notificationPrefs: {
    ...
    morning_mission: { enabled: true, time: '07:15' },
    goal_reminder: { enabled: true, time: '12:00' },
    habit_reminder: { enabled: true, time: '18:00' },
    evening_review: { enabled: true, time: '21:00' },
    motivation: { enabled: false, time: '15:00' },
    deadline: { enabled: true, time: '09:00' },
    daily_reminder_morning: { enabled: true, time: '08:00' },
    daily_summary_midday: { enabled: true, time: '13:00' },
    daily_summary_evening: { enabled: true, time: '20:30' },
  },
...
class Store {
   ...
   */
  getMoneyStats(days = 30) {
    const all = this.state.transactions;
    const entries = days > 0
      ? all.filter((t) => new Date(t.date).getTime() >= Date.now() - days * 86400000)
      : all;
    ...
    // Solde global toutes périodes confondues (pas seulement la fenêtre).
  ...
  getNotificationPrefs() {
    return this.state.notificationPrefs;
  }
