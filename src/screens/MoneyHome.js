...
import { moneyCategoryMap, expenseCategories } from '../data/moneyCategories.js';
...
function transactionRow(tx, accountsById, showAccount) {
...
export function MoneyHome() {
  ...
  const accountTabsHtml = accounts.length > 1
    ...
    <div class="tab-row mind-tab-row" role="tablist" id="account-tabs" style="margin-top:var(--sp-3)">
      ...
      ${accounts.map((a) => `<button class="tab-btn ${activeAccountId === a.id ? 'is-active' : ''}" data-account="${a.id}">${a.name}</button>`).join('')}
    </div>` : '';
  ...
  if (activeTab === 'apercu') {
    ...
    budgetBlock.innerHTML = `
      ...
      ${budgetProgress.length === 0 ? `<p style="font-size:var(--fs-sm);color:var(--text-tertiary)">Aucun budget défini pour l'instant.</p>` : budgetProgress.map((b) => {
        const cat = moneyCategoryMap[b.category] ?? moneyCategoryMap.autre;
    ...
    if (transactions.length === 0) {
      ...
      transactions.slice(0, 5).forEach((tx) => section.appendChild(transactionRow(tx, accountsById, activeAccountId === 'all')));
    }
    ...
    if (transactions.length === 0) {
      ...
      transactions.forEach((tx) => list.appendChild(transactionRow(tx, accountsById, activeAccountId === 'all')));
    }
