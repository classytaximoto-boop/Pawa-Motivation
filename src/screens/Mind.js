...
import { coachStyles } from '../data/coachStyles.js';
import { PawaCoach } from '../components/PawaCoach.js';
import { antiLazynessMethods, wakeUpReminders, closingLine, randomAntiLazynessMethod } from '../data/antiLazyness.js';

// État local de l'écran (pas persisté) : onglet actif + fenêtre de stats + humeur sélectionnée dans le formulaire.
let activeTab = 'checkin'; // checkin | behaviorJournal | history | stats | problems | leadership | quotes | xp | antilazyness
...
// État local pour l'onglet Anti-Paresse : coach choisi (null = pas encore choisi
// pour ce passage) + méthode actuellement affichée.
let selectedCoach = null; // null | 'military' | 'motivating' | 'friendly'
let currentAntiLazynessMethod = null;
...
function renderCheckin(screen) {
  screen.innerHTML = `
    <form id="emotion-form" novalidate>
      ...
      <div class="form-group" style="margin-top: var(--sp-4)">
        <label class="form-label">🧠 Behavior du jour <span class="optional">(optionnel mais recommandé)</span></label>
        <p class="detail-desc" style="margin:4px 0 10px">Ne choisis pas ce que tu voudrais être : indique honnêtement les mauvais comportements que tu as remarqués aujourd'hui.</p>
        <div class="category-scroll" id="bad-traits-picker">
          ${[
            ['impulsivite','⚡ Impulsif'],['colere','🔥 Colère'],['ego','🪞 Ego'],['mensonge','🎭 Mensonge'],['procrastination','⏳ Procrastination'],['validation','🎯 Validation'],['evitement','🧱 Évitement'],['jalousie','💚 Jalousie'],['peur','😨 Peur'],['arrogance','👑 Arrogance'],['manque_discipline','📉 Discipline faible'],['agressivite','💢 Agressivité'],['manipulable','🪤 Trop influençable'],['surcontrole','🎯 Surcontrôle'],['retrait','🌑 Retrait social']
          ].map(([id,label]) => `<button type="button" class="category-chip" data-bad-trait="${id}">${label}</button>`).join('')}
        </div>
      </div>

      <div class="form-group" style="margin-top: var(--sp-4)">
        <label class="form-label" for="f-recent-speech">🗣️ Ce que j'ai dit récemment <span class="optional">(optionnel)</span></label>
        <textarea class="form-textarea" id="f-recent-speech" name="recentSpeech" placeholder="Écris une phrase ou quelques mots que tu as réellement dits récemment. L'analyse cherchera des signaux de comportement."></textarea>
      </div>

      <div class="form-group" style="margin-top: var(--sp-4)">
        <label class="form-label" for="f-situation">Situation <span class="optional">(optionnel)</span></label>
        <textarea class="form-textarea" id="f-situation" name="situation" placeholder="Avec qui / dans quelle situation ?"></textarea>
      </div>
  ...
  screen.querySelectorAll('[data-bad-trait]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const id = btn.dataset.badTrait;
      selectedBadTraits = selectedBadTraits.includes(id)
        ? selectedBadTraits.filter(x => x !== id)
        : [...selectedBadTraits, id];
      btn.classList.toggle('is-active', selectedBadTraits.includes(id));
    });
  });
  ...
  screen.querySelector('#emotion-form').addEventListener('submit', (e) => {
    ...
    store.createBehaviorCheckin({
      badTraits: selectedBadTraits,
      recentSpeech: formData.get('recentSpeech')?.toString() ?? '',
...
function behaviorTraitLabel(id) {
  const labels = {
    impulsivite:'⚡ Impulsivité', colere:'🔥 Colère', ego:'🪞 Ego', mensonge:'🎭 Mensonge',
    procrastination:'⏳ Procrastination', validation:'🎯 Recherche de validation', evitement:'🧱 Évitement',
    jalousie:'💚 Jalousie', peur:'😨 Peur', arrogance:'👑 Arrogance', manque_discipline:'📉 Discipline faible',
    agressivite:'💢 Agressivité', manipulable:'🪤 Influençable', surcontrole:'🎯 Surcontrôle', retrait:'🌑 Retrait social'
  };
  return labels[id] || id;
}

function renderBehaviorJournal(screen) {
  const behaviors = store.listBehaviorCheckins();
  const emotions = store.listEmotionEntries();
  const combined = [
    ...behaviors.map(e => ({ kind:'behavior', date:e.date || e.createdAt, data:e })),
    ...emotions.map(e => ({ kind:'emotion', date:e.date || e.createdAt, data:e })),
  ].sort((a,b) => new Date(b.date) - new Date(a.date));
  const todayKey = new Date().toLocaleDateString('fr-CA');
  const todayCount = combined.filter(e => new Date(e.date).toLocaleDateString('fr-CA') === todayKey).length;
  const grouped = {};
  combined.forEach(e => {
    const key = new Date(e.date).toLocaleDateString('fr-FR', {weekday:'long', day:'2-digit', month:'long', year:'numeric'});
    (grouped[key] ||= []).push(e);
  });
  ...
  screen.innerHTML = `
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;">
        <div><strong>📓 Journal quotidien de Pawa</strong><p class="detail-desc" style="margin-top:4px">Tout ce que tu enregistres apparaît ici dans l'ordre : humeur, émotion, comportement, paroles et situation.</p></div>
        <span class="chip">${todayCount} aujourd’hui</span>
      </div>
      <button type="button" class="btn-primary" id="quick-behavior" style="width:100%;margin-top:var(--sp-3)">＋ Enregistrer un mouvement maintenant</button>
    </div>
    ${combined.length ? Object.entries(grouped).map(([day, list]) => `
      <div class="card__label" style="margin-top:var(--sp-5);text-transform:capitalize">${day}</div>
      <div style="display:flex;flex-direction:column;gap:10px">
        ${list.map(e => {
          const d=e.data;
          if(e.kind==='emotion') {
            const meta=moodMap[d.mood] || moodMap.neutral;
            return `<div class="card card--tight"><div style="display:flex;justify-content:space-between;gap:8px"><div><strong>${new Date(e.date).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</strong> · ${meta.emoji} ${escapeHtml(meta.label)}</div><span class="chip">Mood</span></div><div class="detail-desc" style="margin-top:7px">Motivation ${d.motivation ?? '—'} · Énergie ${d.energy ?? '—'} · Stress ${d.stress ?? '—'} · Intensité ${d.intensity ?? '—'}</div>${d.cause?`<div class="detail-desc" style="margin-top:5px">Cause : ${escapeHtml(d.cause)}</div>`:''}${d.text?`<div style="margin-top:7px">${escapeHtml(d.text)}</div>`:''}</div>`;
          }
          return `<div class="card card--tight"><div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start"><div><strong>${new Date(e.date).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</strong><span class="chip" style="margin-left:7px">Behavior</span></div><button class="icon-btn icon-btn--danger" data-delete-behavior="${d.id}" aria-label="Supprimer">${icons.trash}</button></div>${d.situation?`<div class="detail-desc" style="margin-top:6px">Situation : ${escapeHtml(d.situation)}</div>`:''}${(d.badTraits||[]).length?`<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">${d.badTraits.map(t=>`<span class="chip">⚠️ ${escapeHtml(behaviorTraitLabel(t))}</span>`).join('')}</div>`:'<div class="detail-desc" style="margin-top:7px">Observation comportementale sans mauvais trait sélectionné.</div>'}${d.recentSpeech?`<div style="margin-top:9px;padding:9px;border-left:3px solid var(--accent-500);background:var(--surface-2)"><strong>🗣️ Dit :</strong> « ${escapeHtml(d.recentSpeech)} »</div>`:''}</div>`;
        }).join('')}
      </div>`).join('') : `
      <div class="state-block" style="padding-top:var(--sp-8)">
        ${icons.mind.replace('<svg ', '<svg class="state-block__icon" ')}
        <h2 class="state-block__title">Journal vide</h2>
        <p class="state-block__desc">Enregistre une observation à chaque moment important. Tu peux le faire autant de fois par jour que nécessaire.</p>
      </div>`}
  `;

  screen.appendChild(PawaCoach());

  screen.querySelector('#quick-behavior')?.addEventListener('click', () => {
    activeTab = 'checkin';
    el.replaceWith(Mind());
  });
  screen.querySelectorAll('[data-delete-behavior]').forEach(btn => btn.addEventListener('click', () => {
    document.body.appendChild(mindDeleteConfirmSheet(
      {title:'Supprimer cette observation ?', desc:'Cette entrée comportementale sera supprimée définitivement.'},
      () => { store.deleteBehaviorCheckin(btn.dataset.deleteBehavior); router.navigate('/mind'); }
    ));
  }));
}
...
function renderAntiLazyness(screen) {
  // Étape 1 : pas encore de coach choisi pour ce passage -> on demande le tempérament.
  if (!selectedCoach) {
    screen.innerHTML = `
      <div class="state-block" style="padding-top: var(--sp-4)">
        ${icons.mind.replace('<svg ', '<svg class="state-block__icon" ')}
        <h2 class="state-block__title">Un coup de mou ?</h2>
        <p class="state-block__desc">Choisis le style de coach qui va te bousculer (ou t'encourager) maintenant.</p>
      </div>
      <div class="mood-picker" id="coach-style-picker" style="margin-top: var(--sp-4)">
        ${coachStyles.map((c) => `
          <button type="button" class="mood-picker-item" data-coach="${c.id}">
            <span class="mood-picker-item__emoji">${c.emoji}</span>
            <span>${c.label}</span>
          </button>`).join('')}
      </div>
    `;
    screen.querySelectorAll('[data-coach]').forEach((btn) => {
      btn.addEventListener('click', () => {
        selectedCoach = btn.dataset.coach;
        currentAntiLazynessMethod = randomAntiLazynessMethod();
        renderAntiLazyness(screen);
      });
    });
    return;
  }

  // Étape 2 : coach choisi -> on affiche le message du jour + rappel lever-tôt.
  if (!currentAntiLazynessMethod) currentAntiLazynessMethod = randomAntiLazynessMethod();
  const method = currentAntiLazynessMethod;
  const coachMeta = coachStyles.find((c) => c.id === selectedCoach);

  screen.innerHTML = `
    <div class="category-scroll" style="margin-bottom: var(--sp-4)">
      <button type="button" class="category-chip is-active" id="antilazyness-change-coach">${coachMeta.emoji} ${coachMeta.label} · changer</button>
    </div>

    <section class="card quote-card quote-card--featured" aria-label="Message anti-paresse">
      ${icons.sparkles.replace('<svg ', '<svg class="quote-card__icon" ')}
      <div class="card__label">${method.title}</div>
      <p class="quote-card__text" style="white-space: pre-line">${method.messages[selectedCoach]}</p>
    </section>

    <section class="card" style="margin-top: var(--sp-4)">
      <div class="card__label">Lève-toi tôt</div>
      <p class="quote-card__text" style="white-space: pre-line">${wakeUpReminders[selectedCoach]}</p>
      <div class="form-group" style="margin-top: var(--sp-3)">
        <label class="form-label" for="antilazyness-wake-time">Heure du rappel</label>
        <input class="form-input" type="time" id="antilazyness-wake-time" value="06:00" />
      </div>
      <button type="button" class="btn-secondary" id="antilazyness-set-alarm" style="width:100%; margin-top: var(--sp-3)">Créer le rappel</button>
    </section>

    <div class="mono" style="text-align:center; color:var(--text-tertiary); margin-top: var(--sp-4); font-style: italic;">
      ${closingLine[selectedCoach]}
    </div>

    <div class="quote-card__actions" style="justify-content:center; margin-top: var(--sp-4)">
      <button class="chip" id="antilazyness-next-btn">${icons.sparkles}Un autre message</button>
    </div>
  `;

  screen.querySelector('#antilazyness-change-coach').addEventListener('click', () => {
    selectedCoach = null;
    currentAntiLazynessMethod = null;
    renderAntiLazyness(screen);
  });

  screen.querySelector('#antilazyness-next-btn').addEventListener('click', () => {
    currentAntiLazynessMethod = randomAntiLazynessMethod();
    renderAntiLazyness(screen);
  });

  screen.querySelector('#antilazyness-set-alarm').addEventListener('click', async () => {
    const timeVal = screen.querySelector('#antilazyness-wake-time').value || '06:00';
    const btn = screen.querySelector('#antilazyness-set-alarm');

    if (!('Notification' in window)) {
      btn.textContent = 'Notifications non supportées sur cet appareil';
      return;
    }
    ...
    if (permission !== 'granted') {
      btn.textContent = 'Autorisation refusée — active les notifications';
      return;
    }
    ...
    setTimeout(() => {
      new Notification('Lève-toi — Anti-Paresse', {
        body: wakeUpReminders[selectedCoach],
      });
    }, delayMs);

    btn.textContent = `Rappel programmé pour ${timeVal}`;
    btn.disabled = true;
  });
}
...
export function Mind() {
  ...
  tabs.innerHTML = `
    ...
    <button class="tab-btn ${activeTab === 'behaviorJournal' ? 'is-active' : ''}" data-tab="behaviorJournal">Behavior journal</button>
    ...
    <button class="tab-btn ${activeTab === 'antilazyness' ? 'is-active' : ''}" data-tab="antilazyness">Anti-Paresse</button>
  ...
  else if (activeTab === 'behaviorJournal') renderBehaviorJournal(screen);
  ...
  else if (activeTab === 'antilazyness') renderAntiLazyness(screen);
