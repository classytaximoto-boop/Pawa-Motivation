...
import { openSkillsAddForm } from './Skills.js';
import { developmentCategories, suggestDevelopmentSkills } from '../data/developmentSkills.js';
...
let agendaTitle = 'Liste des tâches quotidiennes'; // titre modifiable du bandeau, mémoire vive
let editingTitle = false; // true quand le champ titre est en édition
let pendingDeleteId = null; // id de la tâche en attente de confirmation de suppression, ou null

// État local du tiroir "Qu'est-ce que cette tâche développe ?" (Development
// Impact), le temps que le formulaire d'ajout/édition reste ouvert. Remis à
// zéro à chaque ouverture/fermeture de formulaire (voir addBar/onEdit plus
// bas) — jamais persisté tel quel, seul task.development l'est via le store.
let devDrawerOpen = false;
let devSelectedSkills = []; // ids sélectionnés pour la tâche en cours d'ajout/édition
let devSuggested = []; // ids suggérés (non cochés d'office) pour le titre actuellement saisi
 ...
 * `onDelete` reçoit déjà la confirmation gérée par l'appelant (bouton en mode
 * "armé" avant suppression réelle, cf. plus bas) — cette fonction ne fait que
 * déclencher la demande de confirmation, jamais la suppression directe.
 * `onMoveUp`/`onMoveDown` : fonctions de réordonnancement, ou null si le
 * réordonnancement n'est pas proposé pour cette carte (première/dernière).
 */
function taskCard(task, startMin, endMin, { onToggle, onDelete, onEdit, onMoveUp, onMoveDown, onAddSkill }) {
  const card = document.createElement('div');
  card.className = `agenda-block${task.done ? ' is-done' : ''}${task.priority ? ' is-priority' : ''}`;

  card.innerHTML = `
    <button type="button" class="agenda-block__check${task.done ? ' is-done' : ' is-pending'}" aria-label="${task.done ? 'Marquer non terminée' : 'Marquer terminée'}">
    ...
    <div class="agenda-block__body">
      ...
      <button type="button" class="chip agenda-block__skill-btn" style="margin-top:var(--sp-2);">${icons.graduationCap || ''}Compétences</button>
    </div>
    <div class="agenda-block__reorder">
      <button type="button" class="agenda-block__reorder-btn" data-dir="up" aria-label="Monter" ${onMoveUp ? '' : 'disabled'}>${icons.chevronUp || '▲'}</button>
      <button type="button" class="agenda-block__reorder-btn" data-dir="down" aria-label="Descendre" ${onMoveDown ? '' : 'disabled'}>${icons.chevronDown || '▼'}</button>
    </div>
  ...
  card.querySelector('.agenda-block__delete').addEventListener('click', (e) => { e.stopPropagation(); onDelete(task); });
  card.querySelector('.agenda-block__edit').addEventListener('click', (e) => { e.stopPropagation(); onEdit(task); });
  card.querySelector('.agenda-block__skill-btn').addEventListener('click', (e) => { e.stopPropagation(); onAddSkill(); });
  const upBtn = card.querySelector('[data-dir="up"]');
  const downBtn = card.querySelector('[data-dir="down"]');
  if (onMoveUp) upBtn.addEventListener('click', (e) => { e.stopPropagation(); onMoveUp(task.id); });
  if (onMoveDown) downBtn.addEventListener('click', (e) => { e.stopPropagation(); onMoveDown(task.id); });
  return card;
}

/**
 * Carte de confirmation affichée à la place d'une tâche quand sa suppression
 * est en attente de confirmation (évite les suppressions accidentelles en un
 * clic). "Annuler" désarme sans rien supprimer ; "Supprimer" appelle le store.
 */
function deleteConfirmCard(task, rerender) {
  const card = document.createElement('div');
  card.className = 'agenda-block agenda-block--confirm-delete';
  card.innerHTML = `
    <div class="agenda-block__body">
      <div class="agenda-block__title">Supprimer « ${task.title} » ?</div>
      <p class="state-block__desc" style="margin:var(--sp-1) 0 0">Cette action est définitive.</p>
    </div>
    <div class="form-actions">
      <button type="button" class="btn-secondary" id="confirm-cancel">Annuler</button>
      <button type="button" class="btn-primary" id="confirm-delete" style="background:var(--danger-500, #dc2626);border-color:var(--danger-500, #dc2626)">${icons.trash || ''}Supprimer</button>
    </div>
  `;
  card.querySelector('#confirm-cancel').addEventListener('click', (e) => {
    e.stopPropagation();
  ...
  card.querySelector('#confirm-delete').addEventListener('click', (e) => {
    e.stopPropagation();
    store.deleteAgendaTask(task.id);
    pendingDeleteId = null;
    rerender();
  });
  return card;
...
/**
 * Contenu du tiroir "Qu'est-ce que cette tâche développe ?" (Development
 * Impact). Rendu à part de timeRangeForm() pour pouvoir le re-rendre seul
 * (ouverture/fermeture, coche d'un skill, suggestion) sans perdre la saisie
 * en cours dans les autres champs du formulaire.
 */
function renderDevelopmentDrawer(container) {
  const suggestedNotYetPicked = devSuggested.filter((id) => !devSelectedSkills.includes(id));
  container.innerHTML = `
    <details class="dev-drawer" ${devDrawerOpen ? 'open' : ''}>
      <summary class="dev-drawer__summary">
        <span class="dev-drawer__chevron">${icons.chevronDown || '▾'}</span>
        <span>${icons.graduationCap || ''}Qu'est-ce que cette tâche développe ?</span>
        <span class="dev-drawer__optional">Optionnel</span>
      </summary>
      <div class="dev-drawer__body">
        <p class="detail-desc" style="margin:var(--sp-2) 0 var(--sp-3) 0;">Optionnel — choisis ce que cette tâche t'aide à développer. Rien à cocher si tu ne sais pas encore.</p>
        ${suggestedNotYetPicked.length ? `
        <div class="dev-drawer__suggested">
          <span class="detail-desc" style="margin:0;">Suggéré :</span>
          ${suggestedNotYetPicked.map((id) => {
            const meta = developmentCategories.flatMap((c) => c.skills).find((s) => s.id === id);
            if (!meta) return '';
            return `<button type="button" class="category-chip dev-suggest-chip" data-suggest-add="${id}">+ ${meta.label}</button>`;
          }).join('')}
        </div>` : ''}
        ${developmentCategories.map((cat) => `
          <div class="dev-drawer__category">
            <div class="dev-drawer__category-label">${cat.label}</div>
            <div class="dev-drawer__skills">
              ${cat.skills.map((s) => `
                <label class="dev-skill-check">
                  <input type="checkbox" data-dev-skill="${s.id}" ${devSelectedSkills.includes(s.id) ? 'checked' : ''} />
                  <span>${s.label}</span>
                </label>
              `).join('')}
            </div>
  ...
  const details = container.querySelector('.dev-drawer');
  details.addEventListener('toggle', () => { devDrawerOpen = details.open; });

  container.querySelectorAll('[data-dev-skill]').forEach((input) => {
    input.addEventListener('change', () => {
      const id = input.getAttribute('data-dev-skill');
      if (input.checked) {
        if (!devSelectedSkills.includes(id)) devSelectedSkills.push(id);
      } else {
        devSelectedSkills = devSelectedSkills.filter((s) => s !== id);
      }
    });
  });
  ...
  container.querySelectorAll('[data-suggest-add]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const id = btn.getAttribute('data-suggest-add');
      if (!devSelectedSkills.includes(id)) devSelectedSkills.push(id);
      devDrawerOpen = true;
      renderDevelopmentDrawer(container);
    });
  });
}
 ...
 * `initialDevelopment` : { skills: string[] } de la tâche éditée, ou null en création.
 */
function timeRangeForm({ title, initialStart, initialEnd, initialTitle = '', initialNote = '', initialPriority = false, initialDevelopment = null, submitLabel, onSubmit, onCancel }) {
  devSelectedSkills = initialDevelopment?.skills ? [...initialDevelopment.skills] : [];
  devDrawerOpen = devSelectedSkills.length > 0;
  devSuggested = suggestDevelopmentSkills(initialTitle);
  ...
  form.innerHTML = `
    ...
    <div class="dev-drawer-mount" style="margin-top:var(--sp-3)"></div>
  ...
  const devMount = form.querySelector('.dev-drawer-mount');
  renderDevelopmentDrawer(devMount);

  // Re-calcule les suggestions quand le titre change, sans perdre la
  // sélection déjà cochée à la main (voir §9 — l'utilisateur garde le
  // contrôle, on ne coche jamais automatiquement).
  const titleInput = form.querySelector('#f-agenda-title');
  titleInput.addEventListener('input', () => {
    devSuggested = suggestDevelopmentSkills(titleInput.value);
    renderDevelopmentDrawer(devMount);
  });
  ...
  form.addEventListener('submit', (e) => {
    ...
    onSubmit({
      ...
      startMin,
      endMin,
      development: devSelectedSkills.length ? { skills: [...devSelectedSkills] } : null,
    });
  });
...
/** Bandeau titre : cliquable pour éditer le nom de la liste. */
function titleBanner(rerender) {
  const banner = document.createElement('div');
  banner.className = 'agenda-title-banner';
  if (editingTitle) {
    const form = document.createElement('form');
    form.className = 'agenda-title-banner__form';
    form.innerHTML = `
      <span class="agenda-title-banner__icon">${icons.clipboard || icons.check || ''}</span>
      <input class="agenda-title-banner__input" type="text" name="agendaTitle" value="${agendaTitle}" maxlength="60" required />
    `;
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      const value = new FormData(form).get('agendaTitle').trim();
      if (value) agendaTitle = value;
      editingTitle = false;
      rerender();
    });
    ...
    banner.innerHTML = `
      <span class="agenda-title-banner__icon">${icons.clipboard || icons.check || ''}</span>
      <h1 class="agenda-title-banner__title">${agendaTitle}</h1>
      <button type="button" class="agenda-title-banner__edit" aria-label="Modifier le titre">${icons.edit || '✎'}</button>
    `;
    banner.querySelector('.agenda-title-banner__edit').addEventListener('click', () => {
      editingTitle = true;
      rerender();
    });
  }
  ...
  buildMonthGrid(calendarMonthKey).forEach((dateKey) => {
    ...
    if (!dateKey) {
      ...
      cell.addEventListener('click', () => {
        currentDate = dateKey;
        pendingDeleteId = null; // change de jour : on désarme une éventuelle confirmation en attente
        rerender();
      });
    }
...
function dayView(el, rerender) {
  ...
  // Bandeau titre modifiable + mini-calendrier, côte à côte en tête d'écran.
  const header = document.createElement('div');
  header.className = 'agenda-header-row';
  header.appendChild(titleBanner(rerender));
  header.appendChild(monthCalendar(rerender));
  ...
  if (addingTask) {
    wrap.appendChild(timeRangeForm({
      ...
      submitLabel: 'Ajouter',
      onSubmit: ({ title, note, priority, startMin, endMin, development }) => {
        store.addAgendaTask({ date: currentDate, hour: Math.floor(startMin / 60), startMin, endMin, title, note, priority, development });
        addingTask = null;
        rerender();
      },
  ...
  if (editingTask) {
    ...
    wrap.appendChild(timeRangeForm({
      ...
      initialDevelopment: editingTask.development || null,
  ...
  // Cadres 2-7 : liste 1 colonne des tâches du jour.
  // Ordre : `store.getAgendaTasksForDate` renvoie déjà les tâches triées par
  // ordre manuel (réordonnancement) puis par heure de début en repli — ne pas
  // re-trier ici, sous peine d'ignorer le réordonnancement de l'utilisateur.
  // L'heure de chaque tâche reste libre à la minute (choisie dans le formulaire) —
  // ce n'est pas une timeline à créneaux fixes, juste un rendu en liste de cartes.
  if (tasks.length === 0) {
    ...
    tasks.forEach((task, index) => {
      ...
      if (pendingDeleteId === task.id) {
        list.appendChild(deleteConfirmCard(task, rerender));
        return;
      }
      list.appendChild(taskCard(task, startMin, endMin, {
        onToggle: (id) => {
        ...
        onDelete: (t) => { pendingDeleteId = t.id; rerender(); },
        onEdit: (t) => { editingTaskId = t.id; addingTask = null; pendingDeleteId = null; rerender(); },
        onMoveUp: index > 0 ? (id) => { store.moveAgendaTask(id, -1); rerender(); } : null,
        onMoveDown: index < tasks.length - 1 ? (id) => { store.moveAgendaTask(id, 1); rerender(); } : null,
        onAddSkill: () => { openSkillsAddForm(); router.navigate('/developpement'); },
      }));
    });
  ...
  // Compteurs : tâches réalisées / non réalisées, calculés automatiquement.
  const doneCount = tasks.filter((t) => t.done).length;
  const notDoneCount = tasks.length - doneCount;
  const counters = document.createElement('div');
  counters.className = 'agenda-counters';
  counters.innerHTML = `
    <div class="agenda-counters__item">
      <span class="agenda-counters__label">Nombre de tâches réalisées</span>
      <span class="agenda-counters__value">${doneCount}</span>
    </div>
    <div class="agenda-counters__item">
      <span class="agenda-counters__label">Nombre de tâches non réalisées</span>
      <span class="agenda-counters__value">${notDoneCount}</span>
    </div>
  `;
  wrap.appendChild(counters);

  // Nav jour : déplacée en bas, flèches rondes + libellé de date au centre.
  const nav = document.createElement('div');
  nav.className = 'agenda-day-nav agenda-day-nav--bottom';
  nav.innerHTML = `
    <button type="button" class="agenda-day-nav__arrow" id="agenda-prev" aria-label="Jour précédent">${icons.chevronLeft || '‹'}</button>
    <div class="agenda-day-nav__label">${fmtDateLabel(currentDate)}</div>
    <button type="button" class="agenda-day-nav__arrow" id="agenda-next" aria-label="Jour suivant">${icons.chevronRight || '›'}</button>
  `;
  nav.querySelector('#agenda-prev').addEventListener('click', () => { currentDate = shiftDate(currentDate, -1); pendingDeleteId = null; rerender(); });
  nav.querySelector('#agenda-next').addEventListener('click', () => { currentDate = shiftDate(currentDate, 1); pendingDeleteId = null; rerender(); });
