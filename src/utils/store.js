...
import { pickRandomLesson } from '../data/dailyLessons.js';
import { verificationBaseCategories } from '../data/verificationCategories.js';
import { detectSkillsForCompletion, splitXpAcrossSkills, xpToLevel, classifyDifficulty, resolveDevelopmentSkills } from './skillEngine.js';
import { pickConfidenceTrigger } from '../data/confidenceTriggers.js';
import { pickSelfEsteemTrigger } from '../data/selfEsteemTriggers.js';
import { getPawaAnalytics } from '../data/pawaAnalytics.js';
...
const defaultState = {
  ...
  // Historique append-only du streak : un point par check-in réel (voir
  // checkInToday()), { date: 'YYYY-MM-DD', streak }. Sert uniquement à
  // calculer un delta honnête "streak vs le mois dernier" sur Home
  // Dashboard (getStreakMonthDelta()) — user.streak reste la seule source
  // de vérité pour l'affichage du streak courant partout ailleurs, cet
  // historique n'est jamais lu que pour ce calcul de delta.
  streakHistory: [],
  mood: {
  ...
  // Leçon journalière (Marketing, Négociation, Management, etc.) — voir data/dailyLessons.js
  // et _ensureDailyLesson(). dailyLesson embarque directement la leçon complète (pas juste
  // son id) pour rester affichable même si le pool évolue plus tard sans casser l'historique.
  dailyLesson: null,
  dailyLessonDate: null, // 'YYYY-MM-DD' — date à laquelle dailyLesson a été tiré
  dailyLessonRead: false, // passe à true quand l'utilisateur marque la leçon du jour comme lue (crédite l'XP une seule fois)
  mainGoalToday: {
  ...
  behaviorCheckins: [], // { id, date, badTraits[], recentSpeech, situation, createdAt } — journal comportemental append-only
  problems: [], // Problem[]
  leadershipScores: [], // LeadershipScore[]
  xpHistory: [], // XPHistory[] — { id, amount, source, label, date }
  // CHARACTER JOURNEY — preuves structurées pour faire évoluer le profil de
  // confiance/leadership. Chaque action réelle peut créditer plusieurs dimensions.
  developmentProfile: {
    confidence: 0, selfEsteem: 0, leadership: 0, charisma: 0, calm: 0,
    discipline: 0, communication: 0, assertiveness: 0, courage: 0,
    resilience: 0, decisionMaking: 0, socialCourage: 0, emotionalControl: 0,
    actions: 0,
  },
  developmentEvidence: [], // { id, challengeId, type, label, xp, baseXp, multiplier, difficulty, dimensions, reflection, date, createdAt }
  // MODE DE PAWA — modes activés par l'utilisateur. Les données sources ne sont jamais modifiées.
  activePawaModes: [],
  developmentChallengeState: {
    dailyDate: null,
    dailyXp: 0,
    dailyActions: 0,
    streak: 0,
  ...
  accomplishments: [], // Accomplishment[] — { id, text, date, confidence, selfEsteem, points, createdAt } — voir ACCOMPLISHMENTS plus bas
  // CONFIDENCE / SELF-ESTEEM — preuves distinctes (voir CONFIDENCE & SELF-ESTEEM
  // ENGINES plus bas). Ne remplacent pas accomplishments : ce sont deux journaux
  // d'evidence séparés, l'un pour la capacité ("I CAN"), l'autre pour la valeur
  // personnelle ("I HAVE VALUE"). Absents des anciennes sauvegardes → toujours
  // initialisés à [] via le merge superficiel de load(), sans migration requise.
  confidenceEvidence: [], // ConfidenceEvidence[] — { id, type, label, sourceId, date, createdAt }
  selfEsteemEvidence: [], // SelfEsteemEvidence[] — { id, type, label, sourceId, date, createdAt }
  businessPlan: {
  ...
  notificationPrefs: {
    ...
    // Rappel pour lire la leçon du jour (voir data/dailyLessons.js) si elle n'a pas
    // encore été marquée comme lue — voir le cas particulier dans checkDueNotifications().
    daily_lesson: { enabled: true, time: '09:00', sound: true, vibrate: true },
  },
  ...
  // + verification optionnel sur une tâche (PAWA BELOHA) : { enabled, category, items: [{id, title, checked}], completedAt }
  // Ajouté par addVerificationConfig()/toggleVerificationItem() etc. — absent tant que l'utilisateur n'a jamais activé
  // la vérification pour cette tâche (aucune migration nécessaire, `task.verification` est simplement undefined).

  // Catégories personnalisées créées par l'utilisateur pour Pawa Beloha, en plus des 7
  // catégories de base fixes (voir data/verificationCategories.js, jamais modifié ici).
  // Forme : [{ id, label, icon }], mêmes champs que les catégories de base.
  verificationCustomCategories: [],

  // Mode d'affichage clair/sombre — 'dark' (défaut, comportement historique de
  // l'app), 'light', ou 'auto' (suit prefers-color-scheme, voir main.js pour
  // l'application au <html data-theme>). Choix manuel persistant, indépendant
  // du réglage système tant que 'auto' n'est pas choisi.
  themeMode: 'dark',

  // ============================================================
  // PAWA BELOHA — désormais un onglet indépendant (voir data/navigation.js,
  // route /pawa-beloha), plus lié à une tâche Agenda. Le champ `verification`
  // qui existait auparavant sur agendaTasks (voir plus haut) n'est plus créé
  // par aucune UI, mais reste lu tel quel pour ne rien casser si des tâches
  // existantes en contiennent déjà — pas de migration destructive.
  //
  // pawaBelohaChecklists : PawaBelohaChecklist[]
  //   { id, category (id de catégorie, base ou personnalisée), title,
  //     steps: { avant: Item[], pendant: Item[], apres: Item[] },
  //     currentStep: 'avant' | 'pendant' | 'apres',
  //     verifiedBy, remarks, createdAt, completedAt }
  //   Item = { id, title, checked }
  // ============================================================
  pawaBelohaChecklists: [],
  agendaXp: {
    xp: 0,
    totalEarned: 0,
  },
...
class Store {
  ...
  /**
   * Régénère la leçon du jour (voir data/dailyLessons.js) dès que la date du
   * jour change par rapport à dailyLessonDate. Même logique idempotente que
   * _ensureTodayMissions() : ne touche à rien si la date n'a pas changé, donc
   * ne fait jamais perdre l'état "lue" (dailyLessonRead) en cours de journée.
   */
  _ensureDailyLesson() {
    const todayKey = new Date().toISOString().slice(0, 10);
    if (this.state.dailyLessonDate === todayKey) return;

    const lesson = pickRandomLesson();
    this.state = {
      ...this.state,
      dailyLesson: lesson,
      dailyLessonDate: todayKey,
      dailyLessonRead: false,
    };
    persist(this.state);
  }

  /**
   * Marque la leçon du jour comme lue et crédite l'XP une seule fois par jour
   * (idempotent : rappeler cette méthode alors que dailyLessonRead est déjà
   * true ne fait rien, pour ne pas permettre de farmer l'XP en spammant le
   * bouton "Marquer comme lue").
   */
  markDailyLessonRead() {
    this._ensureDailyLesson();
    if (this.state.dailyLessonRead || !this.state.dailyLesson) return;
    this.set({ dailyLessonRead: true });
    this.addXp(this.state.dailyLesson.xp ?? 15, 'action', `Leçon : ${this.state.dailyLesson.title}`);
  }
  ...
  /** Mode d'affichage actuel — 'dark' | 'light' | 'auto'. */
  getThemeMode() {
    return this.state.themeMode || 'dark';
  }

  /**
   * Change le mode d'affichage. `set()` persiste et notifie déjà les abonnés
   * automatiquement — voir main.js pour l'abonnement qui applique
   * data-theme sur <html> à chaque changement, y compris celui-ci.
   */
  setThemeMode(mode) {
  ...
  toggleMission(id) {
    ...
    // Volontairement PAS de recordEvidenceFromCompletion ici : les missions
    // quotidiennes sont de petites tâches (10-15 xp) tirées aléatoirement —
    // en créer une "preuve" à chaque coche noierait le fil d'Evidence sous
    // des accomplissements mineurs répétitifs (voir §4 ACCOMPLISHMENTS :
    // distinguer minor/meaningful/major/milestone, pas tout élever au même rang).
  }
  ...
  getPawaAnalytics() {
    return getPawaAnalytics(this.state);
  }

  getXpQuality() {
    return this.getPawaAnalytics().xp;
  }
   ...
   */
  /**
   * `development`, optionnel : { skills: string[], primarySkill: string|null }
   * — sélection faite dans le tiroir "Qu'est-ce que cette tâche développe ?".
   * Absent/omis pour une tâche créée sans cette sélection (comportement
   * historique inchangé) ; `task.development` reste alors simplement
   * `undefined`, exactement comme `task.verification` avant l'ajout de
   * Pawa Beloha — aucune migration nécessaire pour les tâches existantes.
   */
  addAgendaTask({ date, hour, duration = 1, startMin, endMin, title, note = '', priority = false, development = null }) {
    const h = Number(hour);
    ...
    // N'ajoute le champ que s'il y a réellement une sélection, pour garder
    // les tâches sans développement identiques à avant (pas de bruit dans le
    // state, pas de `development: { skills: [] }` systématique).
    if (development && Array.isArray(development.skills) && development.skills.length > 0) {
      task.development = { skills: development.skills, primarySkill: development.primarySkill || null };
    }
  ...
  /**
   * Patch générique. Cas particulier pour `development` : si le patch fournit
   * explicitement `development: null` ou `{ skills: [] }` (l'utilisateur a
   * tout décoché), on supprime le champ de la tâche plutôt que d'y laisser un
   * objet vide — une tâche sans sélection ne doit pas se distinguer d'une
   * tâche jamais éditée (voir addAgendaTask).
   */
  updateAgendaTask(id, patch) {
    let normalized = patch;
    if (Object.prototype.hasOwnProperty.call(patch, 'development')) {
      const dev = patch.development;
      const hasSkills = dev && Array.isArray(dev.skills) && dev.skills.length > 0;
      normalized = { ...patch };
      if (hasSkills) {
        normalized.development = { skills: dev.skills, primarySkill: dev.primarySkill || null };
      } else {
        delete normalized.development;
      }
      this.set({
        agendaTasks: this.state.agendaTasks.map((t) => {
          if (t.id !== id) return t;
          const next = { ...t, ...normalized };
          if (!hasSkills) delete next.development;
          return next;
        }),
      });
      return;
  ...
  toggleAgendaTask(id) {
    ...
    // Une tâche Agenda terminée est une action réelle complétée : alimente
    // Evidence/Skills/Confidence comme les autres sources (objectif, projet,
    // habitude) — voir recordEvidenceFromCompletion. sourceId = id de la
    // tâche : une tâche Agenda n'est pas récurrente comme une habitude, donc
    // pas besoin de composer avec une date pour l'anti-doublon.
    if (willBeDone) {
      this.recordEvidenceFromCompletion({
        type: 'task_completed',
        sourceId: id,
        label: task.title,
        xpAmount: xpGained,
        development: task.development || null,
      });
    }
  ...
  // ============================================================
  // PAWA BELOHA — vérification personnalisée liée à une tâche Agenda.
  // Toute la logique passe par updateAgendaTask(id, { verification }) pour
  // rester cohérente avec le reste du système Agenda (persistance, merge
  // partiel, historique) — aucune duplication de stockage séparée.
  // ============================================================

  /** Catégories de base (fixes) + catégories personnalisées créées par l'utilisateur. */
  getVerificationCategories() {
    return [...verificationBaseCategories, ...this.state.verificationCustomCategories];
  }

  /**
   * Crée une catégorie personnalisée (via "+ Ajouter une catégorie") et la
   * renvoie. Le label est utilisé tel quel ; l'id est dérivé et rendu unique
   * si un id identique existe déjà (catégorie de base ou personnalisée).
   */
  addVerificationCategory(label) {
    const trimmed = (label || '').trim();
    if (!trimmed) return null;
    const baseId = trimmed.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || 'categorie';
    const existingIds = new Set(this.getVerificationCategories().map((c) => c.id));
    let id = baseId;
    let n = 2;
    while (existingIds.has(id)) { id = `${baseId}_${n}`; n += 1; }
    const category = { id, label: trimmed, icon: 'folder' };
    this.set({ verificationCustomCategories: [...this.state.verificationCustomCategories, category] });
    return category;
  }

  /**
   * Active ou remplace la configuration de vérification d'une tâche.
   * Toujours appelée avec une config complète (category + items) — pour un
   * simple changement d'activation, voir setVerificationEnabled().
   */
  setVerificationConfig(taskId, { category, items }) {
    this.updateAgendaTask(taskId, {
      verification: {
        enabled: true,
        category,
        items: (items || []).map((it) => ({
  ...
  /**
   * Bascule enabled à true/false. Passer à false ne supprime pas les items
   * déjà configurés (ils restent en mémoire si l'utilisateur réactive plus
   * tard) — seul `enabled` change, cohérent avec "Non → rien ne s'affiche,
   * mais rien n'est perdu si l'utilisateur change d'avis".
   */
  setVerificationEnabled(taskId, enabled) {
    const task = this.state.agendaTasks.find((t) => t.id === taskId);
    if (!task) return;
    const current = task.verification || { category: null, items: [], completedAt: null };
    this.updateAgendaTask(taskId, { verification: { ...current, enabled: !!enabled } });
  }

  /** Ajoute un item de vérification (checklist vide au départ, jamais pré-remplie). */
  addVerificationItem(taskId, title) {
    const trimmed = (title || '').trim();
    if (!trimmed) return;
    const task = this.state.agendaTasks.find((t) => t.id === taskId);
    if (!task || !task.verification) return;
    const items = [...task.verification.items, { id: uid('vitem'), title: trimmed, checked: false }];
    this.updateAgendaTask(taskId, { verification: { ...task.verification, items, completedAt: null } });
  }

  /** Modifie le titre d'un item existant. */
  updateVerificationItem(taskId, itemId, title) {
    const trimmed = (title || '').trim();
    if (!trimmed) return;
    const task = this.state.agendaTasks.find((t) => t.id === taskId);
    if (!task || !task.verification) return;
    const items = task.verification.items.map((it) => (it.id === itemId ? { ...it, title: trimmed } : it));
    this.updateAgendaTask(taskId, { verification: { ...task.verification, items } });
  }

  /** Supprime un item de la checklist. */
  removeVerificationItem(taskId, itemId) {
    const task = this.state.agendaTasks.find((t) => t.id === taskId);
    if (!task || !task.verification) return;
    const items = task.verification.items.filter((it) => it.id !== itemId);
    this.updateAgendaTask(taskId, { verification: { ...task.verification, items } });
  }

  /**
   * Réordonne un item d'un cran vers le haut (-1) ou le bas (+1) dans la
   * checklist de la tâche — même logique que moveAgendaTask() mais localisée
   * à la liste d'items d'une seule tâche.
   */
  moveVerificationItem(taskId, itemId, direction) {
    const task = this.state.agendaTasks.find((t) => t.id === taskId);
    if (!task || !task.verification) return;
    const items = task.verification.items.slice();
  ...
  /**
   * Coche/décoche un item. Si tous les items passent à checked après cette
   * action, marque automatiquement completedAt (horodatage de fin de
   * vérification) ; si un item est décoché ensuite, completedAt repasse à
   * null (la vérification n'est plus "terminée" tant que tout n'est pas coché).
   */
  toggleVerificationItem(taskId, itemId) {
    const task = this.state.agendaTasks.find((t) => t.id === taskId);
    if (!task || !task.verification) return;
    const items = task.verification.items.map((it) => (it.id === itemId ? { ...it, checked: !it.checked } : it));
    const allChecked = items.length > 0 && items.every((it) => it.checked);
    this.updateAgendaTask(taskId, {
      verification: {
        ...task.verification,
        items,
        completedAt: allChecked ? (task.verification.completedAt || new Date().toISOString()) : null,
      },
    });
  }

  /** Résumé {checkedCount, total} pour l'indicateur discret dans la carte de tâche. */
  getVerificationSummary(task) {
    if (!task.verification || !task.verification.enabled) return null;
    const total = task.verification.items.length;
    const checkedCount = task.verification.items.filter((it) => it.checked).length;
    return { checkedCount, total, completedAt: task.verification.completedAt };
  }

  // ============================================================
  // PAWA BELOHA — onglet indépendant. Une "checklist" représente une
  // vérification en cours ou terminée, organisée en 3 étapes (avant/pendant/
  // après), chacune avec sa propre liste d'items cochables construite par
  // l'utilisateur (jamais pré-remplie). getVerificationCategories() et
  // addVerificationCategory() (définies plus haut) sont réutilisées telles
  // quelles — les catégories restent partagées entre l'ancien système de
  // vérification de tâche et ce nouvel onglet, pas de duplication.
  // ============================================================

  /** Liste des checklists Pawa Beloha, plus récentes en premier. */
  listPawaBelohaChecklists() {
    return this.state.pawaBelohaChecklists.slice().sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  }

  getPawaBelohaChecklist(id) {
    return this.state.pawaBelohaChecklists.find((c) => c.id === id) || null;
  }

  /**
   * Crée une nouvelle checklist pour une catégorie donnée. Les 3 étapes
   * démarrent toutes vides — c'est à l'utilisateur d'ajouter ses propres
   * éléments de vérification ensuite, aucun contenu n'est jamais pré-rempli.
   */
  createPawaBelohaChecklist({ category, title }) {
    const checklist = {
      id: uid('pawabeloha'),
      category,
      title: (title || '').trim(),
      steps: { avant: [], pendant: [], apres: [] },
      currentStep: 'avant',
      verifiedBy: '',
      remarks: '',
      createdAt: new Date().toISOString(),
    ...
    this.set({ pawaBelohaChecklists: [...this.state.pawaBelohaChecklists, checklist] });
    return checklist;
  }

  /** Merge partiel sur une checklist — même pattern que updateAgendaTask(). */
  updatePawaBelohaChecklist(id, patch) {
    this.set({
      pawaBelohaChecklists: this.state.pawaBelohaChecklists.map((c) => (c.id === id ? { ...c, ...patch } : c)),
    });
  }

  deletePawaBelohaChecklist(id) {
    this.set({ pawaBelohaChecklists: this.state.pawaBelohaChecklists.filter((c) => c.id !== id) });
  }

  /** Change l'étape actuellement affichée (stepper 1-2-3), sans toucher aux items. */
  setPawaBelohaStep(id, step) {
    if (!['avant', 'pendant', 'apres'].includes(step)) return;
    this.updatePawaBelohaChecklist(id, { currentStep: step });
  }

  /** Ajoute un item de vérification à une étape précise de la checklist. */
  addPawaBelohaItem(checklistId, step, title) {
    const trimmed = (title || '').trim();
    if (!trimmed) return;
    const checklist = this.getPawaBelohaChecklist(checklistId);
    if (!checklist) return;
    const items = [...checklist.steps[step], { id: uid('pbitem'), title: trimmed, checked: false }];
    this.updatePawaBelohaChecklist(checklistId, { steps: { ...checklist.steps, [step]: items } });
  }

  updatePawaBelohaItem(checklistId, step, itemId, title) {
    const trimmed = (title || '').trim();
    if (!trimmed) return;
    const checklist = this.getPawaBelohaChecklist(checklistId);
    if (!checklist) return;
    const items = checklist.steps[step].map((it) => (it.id === itemId ? { ...it, title: trimmed } : it));
    this.updatePawaBelohaChecklist(checklistId, { steps: { ...checklist.steps, [step]: items } });
  }

  removePawaBelohaItem(checklistId, step, itemId) {
    const checklist = this.getPawaBelohaChecklist(checklistId);
    if (!checklist) return;
    const items = checklist.steps[step].filter((it) => it.id !== itemId);
    this.updatePawaBelohaChecklist(checklistId, { steps: { ...checklist.steps, [step]: items } });
  }

  movePawaBelohaItem(checklistId, step, itemId, direction) {
    const checklist = this.getPawaBelohaChecklist(checklistId);
    if (!checklist) return;
    const items = checklist.steps[step].slice();
    const idx = items.findIndex((it) => it.id === itemId);
    if (idx === -1) return;
    const targetIdx = idx + direction;
    if (targetIdx < 0 || targetIdx >= items.length) return;
  ...
  /**
   * Coche/décoche un item d'une étape. Ne touche jamais completedAt ici :
   * la checklist n'est marquée terminée que via completePawaBelohaChecklist(),
   * une action explicite de l'utilisateur (bouton "Terminer"), pas un effet
   * de bord automatique du dernier item coché — cohérent avec le fait qu'il y
   * a 3 étapes et que "tout coché sur avant" ne signifie pas "vérification finie".
   */
  togglePawaBelohaItem(checklistId, step, itemId) {
    const checklist = this.getPawaBelohaChecklist(checklistId);
    if (!checklist) return;
    const items = checklist.steps[step].map((it) => (it.id === itemId ? { ...it, checked: !it.checked } : it));
    this.updatePawaBelohaChecklist(checklistId, { steps: { ...checklist.steps, [step]: items } });
  }

  /** Total agrégé {checkedCount, total} sur les 3 étapes confondues. */
  getPawaBelohaSummary(checklist) {
    const allItems = [...checklist.steps.avant, ...checklist.steps.pendant, ...checklist.steps.apres];
  ...
  /**
   * Marque la checklist comme terminée (rapport final) — nécessite que tous
   * les items des 3 étapes soient cochés, sinon ne fait rien (le bouton
   * "Terminer la vérification" de l'écran doit déjà être désactivé dans ce cas,
   * cette vérification est une garde supplémentaire côté store).
   */
  completePawaBelohaChecklist(checklistId, { verifiedBy = '', remarks = '' } = {}) {
    const checklist = this.getPawaBelohaChecklist(checklistId);
    if (!checklist) return;
    const { checkedCount, total } = this.getPawaBelohaSummary(checklist);
    if (total === 0 || checkedCount !== total) return;
    this.updatePawaBelohaChecklist(checklistId, {
  ...
  /**
   * Compare le nombre d'objectifs complétés ce mois-ci vs le mois précédent,
   * pour le delta "+N vs last month" de Quick Stats (Home Dashboard).
   * S'appuie uniquement sur `completedAt`, déjà écrit par toggleGoalStep/
   * deleteGoalStep/markGoalCompleted — aucune nouvelle donnée stockée,
   * aucune migration nécessaire. Les objectifs complétés avant l'ajout de
   * cette méthode ont déjà un `completedAt` (champ existant de longue date),
   * donc le delta est correct dès le premier mois suivant leur complétion —
   * seuls les objectifs jamais horodatés (ne devrait pas arriver en pratique)
   * seraient ignorés du comptage mensuel.
   */
  getGoalCompletionMonthDelta(date = new Date()) {
    const now = new Date(date);
    const rangeThis = this._monthRange(now.getFullYear(), now.getMonth());
    const prevDate = new Date(now.getFullYear(), now.getMonth() - 1, 1);
    const rangePrev = this._monthRange(prevDate.getFullYear(), prevDate.getMonth());

    const countInRange = (range) =>
      this.state.goals.filter((g) => {
        if (g.status !== 'completed' || !g.completedAt) return false;
        const key = this.todayKey(new Date(g.completedAt));
        return key >= range.start && key <= range.end;
      }).length;

    const thisMonth = countInRange(rangeThis);
    const lastMonth = countInRange(rangePrev);
    return { thisMonth, lastMonth, delta: thisMonth - lastMonth };
  }

  /**
   * Compare le nombre de tâches d'agenda complétées ce mois-ci vs le mois
   * précédent, pour le delta "+N vs last month" de la tuile "Tasks
   * completed" (Home Dashboard). Même pattern que
   * getGoalCompletionMonthDelta() : s'appuie uniquement sur `doneAt`, déjà
   * écrit par toggleAgendaTask() sur chaque AgendaTask — aucune
   * nouvelle donnée stockée, aucune migration nécessaire.
   */
  getTasksCompletedMonthDelta(date = new Date()) {
    const now = new Date(date);
    const rangeThis = this._monthRange(now.getFullYear(), now.getMonth());
    const prevDate = new Date(now.getFullYear(), now.getMonth() - 1, 1);
    const rangePrev = this._monthRange(prevDate.getFullYear(), prevDate.getMonth());
    ...
    const countInRange = (range) =>
      this.state.agendaTasks.filter((t) => {
        if (!t.done || !t.doneAt) return false;
        const key = this.todayKey(new Date(t.doneAt));
        return key >= range.start && key <= range.end;
      }).length;
  ...
  toggleGoalStep(goalId, stepId) {
    ...
    if (isNowComplete && !wasCompleted) {
      this.addXp(30, 'objectif', goal.name);
      this.recordEvidenceFromCompletion({ type: 'goal_completed', sourceId: goalId, label: goal.name, xpAmount: 30 });
      // Self-Esteem — "YOU CAN TRUST YOURSELF" : un objectif avec échéance,
      // tenu au plus tard le jour dit, est une promesse envers soi-même
      // respectée. Un objectif sans deadline ne déclenche pas ce trigger —
      // il n'y a pas eu d'engagement de temps à honorer.
      if (goal.deadline && new Date().toISOString().slice(0, 10) <= goal.deadline) {
        this.recordSelfEsteemEvidence({ type: 'promise_kept', sourceId: goalId, label: goal.name });
      }
  ...
  /**
   * Compare le streak actuel à sa valeur de fin du mois précédent, pour le
   * delta "+N vs last month" de la tuile "Streak" (Home Dashboard).
   * S'appuie sur streakHistory (voir defaultState + checkInToday()) : le
   * streak "du mois dernier" est le dernier point connu à/avant la fin du
   * mois précédent. Comme streakHistory ne commence à s'écrire qu'à partir
   * du déploiement de cette fonctionnalité, `lastMonth` vaut `null` (pas
   * 0) tant qu'aucun point antérieur au mois courant n'existe encore —
   * pour ne jamais afficher un delta inventé (ex. "+7" alors qu'on ne sait
   * juste pas ce qu'était le streak il y a un mois). L'appelant (Home.js)
   * doit afficher un état neutre ("—" ou tuile sans delta) quand
   * lastMonth === null.
   */
  getStreakMonthDelta(date = new Date()) {
    const now = new Date(date);
    const rangePrev = this._monthRange(now.getFullYear(), now.getMonth() - 1);
    const pointsUpToPrevMonthEnd = this.state.streakHistory
      .filter((h) => h.date <= rangePrev.end)
      .sort((a, b) => (a.date < b.date ? -1 : 1));
    if (!pointsUpToPrevMonthEnd.length) {
      return { thisMonth: this.state.user.streak, lastMonth: null, delta: null };
    }
    const lastMonth = pointsUpToPrevMonthEnd[pointsUpToPrevMonthEnd.length - 1].streak;
    const thisMonth = this.state.user.streak;
    return { thisMonth, lastMonth, delta: thisMonth - lastMonth };
  }
  ...
  checkInToday() {
    ...
    const hadPreviousStreak = this.state.user.lastCheckIn && this.state.user.streak > 1;
    const wasBroken = this.state.user.lastCheckIn !== yesterday;
    const streak = this.state.user.lastCheckIn === yesterday ? this.state.user.streak + 1 : 1;
    const todayKey = this.todayKey();
    // Un seul point par jour (upsert) : un check-in ne peut de toute façon
    // arriver qu'une fois par jour (garde plus haut sur lastCheckIn), mais
    // on reste défensif si checkInToday() est un jour rappelé plusieurs fois.
    const streakHistory = [
      ...this.state.streakHistory.filter((h) => h.date !== todayKey),
      { date: todayKey, streak },
    ];
    this.set({ user: { ...this.state.user, streak, lastCheckIn: today }, streakHistory });
    // Self-Esteem — "YOU CAME BACK" : la série a été rompue puis reprise
    // (pas le tout premier check-in de l'app, sinon tout le monde déclenche
    // ceci dès le jour 1). sourceId = date du jour : un seul comeback par jour.
    if (wasBroken && hadPreviousStreak) {
      this.recordSelfEsteemEvidence({ type: 'comeback', sourceId: `checkin_${today}`, label: 'Tu as repris ta série après une pause.' });
    }
  }
  ...
  /**
   * Résumé émotionnel du jour courant (pour l'encart "ta journée aujourd'hui"
   * sur Home) : humeur dominante des check-ins d'aujourd'hui (celle qui revient
   * le plus souvent ; en cas d'égalité, la plus récente tranche) + moyennes du
   * jour. Ne fait aucune interprétation clinique — sert uniquement à choisir
   * une phrase dans data/moodSummaryPhrases.js.
   */
  getTodayMoodSummary() {
    const todayKey = new Date().toISOString().slice(0, 10);
    const entries = this.state.emotions
      .filter((e) => e.date.slice(0, 10) === todayKey)
      .sort((a, b) => new Date(a.date) - new Date(b.date));

    if (!entries.length) {
      return { count: 0, dominantMood: null, avgIntensity: null, avgStress: null, avgEnergy: null };
    }

    const freq = {};
    entries.forEach((e) => { freq[e.mood] = (freq[e.mood] ?? 0) + 1; });
    let dominantMood = entries[entries.length - 1].mood; // dernier check-in par défaut (départage égalité)
    let bestCount = 0;
    Object.entries(freq).forEach(([mood, count]) => {
      if (count > bestCount) { bestCount = count; dominantMood = mood; }
    });
  ...
  updateProblem(id, patch) {
    ...
    if (nowSolved && !wasSolved) {
      this.addXp(20, 'action', `Problème résolu : ${problem.title}`);
      // Résoudre un problème réel est une preuve de capacité directe — voir
      // CONFIDENCE ENGINE plus bas. Pas de doublon possible : sourceId = id
      // du problème, un problème ne peut être "résolu" qu'une fois par cycle.
      this.recordConfidenceEvidence({ type: 'problem_solved', sourceId: id, label: problem.title });
    } else if (!nowSolved && wasSolved) {
      this.addXp(-20, 'action', `Problème rouvert : ${problem.title}`);
    }
  }
   ...
   * `level` reste saisissable à la main pour une compétence ajoutée
   * manuellement (auto-évaluation initiale) — dès que la compétence reçoit
   * de l'XP via creditSkillXp(), son niveau est ensuite dérivé de l'XP
   * cumulé (voir skillEngine.xpToLevel) et n'est plus modifiable à la main.
   */
  addSkill(fields) {
    const id = uid('sk');
    const now = new Date().toISOString();
    const skill = {
      ...
      xp: 0, // XP cumulé issu de preuves réelles — voir creditSkillXp()
      evidenceCount: 0, // nombre d'accomplissements réels ayant fait progresser cette compétence
      lastImproved: null, // ISO date de la dernière preuve créditée
      relatedAccomplishments: [], // ids d'Accomplishment ayant crédité cette compétence
      note: fields.note?.trim() || '',
      createdAt: now,
      updatedAt: now,
    };
  ...
  /**
   * Compare le nombre de compétences ajoutées ce mois-ci vs le mois
   * précédent, pour le delta "+N vs last month" de la tuile "New skills"
   * (Home Dashboard). Même pattern que getGoalCompletionMonthDelta() :
   * s'appuie uniquement sur `createdAt`, déjà écrit par addSkill() et
   * creditSkillXp() (création implicite d'une compétence) — aucune
   * nouvelle donnée stockée, aucune migration nécessaire.
   */
  getNewSkillsMonthDelta(date = new Date()) {
    const now = new Date(date);
    const rangeThis = this._monthRange(now.getFullYear(), now.getMonth());
    const prevDate = new Date(now.getFullYear(), now.getMonth() - 1, 1);
    const rangePrev = this._monthRange(prevDate.getFullYear(), prevDate.getMonth());
    ...
    const countInRange = (range) =>
      this.state.skills.filter((s) => {
        if (!s.createdAt) return false;
        const key = this.todayKey(new Date(s.createdAt));
        return key >= range.start && key <= range.end;
      }).length;
  ...
  /** Trouve une compétence existante par nom (insensible à la casse), ou null. */
  _findSkillByName(name) {
    const target = name.trim().toLowerCase();
    return this.state.skills.find((s) => s.name.trim().toLowerCase() === target) ?? null;
  }

  /**
   * Crédite de l'XP à une compétence à partir d'une preuve réelle
   * (accomplissement). Crée la compétence si elle n'existe pas encore
   * (extensible sans configuration préalable — voir PHILOSOPHIE §13).
   * Recalcule `level` depuis `xp` cumulé via skillEngine.xpToLevel : à
   * partir du premier crédit, le niveau n'est plus une saisie manuelle.
   * Retourne l'id de la compétence créditée.
   */
  creditSkillXp({ name, category }, amount, accomplishmentId) {
    const now = new Date().toISOString();
    let skill = this._findSkillByName(name);
    if (!skill) {
      const id = uid('sk');
      skill = {
        id,
    ...
    const xp = (skill.xp || 0) + amount;
    const relatedAccomplishments = accomplishmentId
      ? [...(skill.relatedAccomplishments || []), accomplishmentId]
      : (skill.relatedAccomplishments || []);
  ...
  // ============================================================
  // BEHAVIOR CHECK-IN — journal des mauvais comportements + paroles récentes.
  // Les entrées sont append-only : elles n'écrasent aucune donnée existante.
  // ============================================================
  createBehaviorCheckin({ badTraits = [], recentSpeech = '', situation = '' } = {}) {
    const cleanTraits = Array.isArray(badTraits)
      ? [...new Set(badTraits.map(v => String(v || '').trim()).filter(Boolean))].slice(0, 12)
      : [];
    const entry = {
      id: uid('bc'),
      date: new Date().toISOString(),
      createdAt: new Date().toISOString(),
      badTraits: cleanTraits,
      recentSpeech: String(recentSpeech || '').trim().slice(0, 2000),
      situation: String(situation || '').trim().slice(0, 1000),
    };
    this.set({ behaviorCheckins: [entry, ...(this.state.behaviorCheckins || [])].slice(0, 500) });
    return entry;
  }

  listBehaviorCheckins() {
    return (this.state.behaviorCheckins || []).slice().sort((a,b) => new Date(b.createdAt || b.date) - new Date(a.createdAt || a.date));
  }

  deleteBehaviorCheckin(id) {
    const before = this.state.behaviorCheckins || [];
    this.set({ behaviorCheckins: before.filter(e => e.id !== id) });
  }

  // ============================================================
  // CHARACTER JOURNEY — actions réelles qui font progresser le profil.
  // L'XP reste l'XP global de l'app ; ces dimensions servent uniquement à
  // calculer une ressemblance motivante avec les 70 modèles.
  // ============================================================
  recordDevelopmentAction({
    ...
    if (!cleanLabel) return { ok: false, reason: 'missing_label' };

    const now = new Date();
    const today = now.toISOString().slice(0, 10);
    const current = this.state.developmentChallengeState || {};
    const daily = current.dailyDate === today
      ? { ...current }
      : { ...current, dailyDate: today, dailyXp: 0, dailyActions: 0 };

    const idKey = challengeId || cleanLabel.toLowerCase();
    const last = daily.completedIds?.[idKey];
    if (last) {
      const elapsed = now.getTime() - new Date(last).getTime();
      if (elapsed < Math.max(1, cooldownHours) * 3600000) {
        const hoursLeft = Math.max(1, Math.ceil((cooldownHours * 3600000 - elapsed) / 3600000));
        return { ok: false, reason: 'cooldown', hoursLeft };
      }
    }
    if ((daily.dailyActions || 0) >= dailyLimit) {
      return { ok: false, reason: 'daily_limit', limit: dailyLimit };
    }

    const baseXp = Math.max(1, Math.round(Number(xp) || 1));
    const remainingDailyXp = Math.max(0, maxDailyXp - (daily.dailyXp || 0));
    if (remainingDailyXp <= 0) return { ok: false, reason: 'daily_xp_cap', maxDailyXp };

    // Les actions répétées restent utiles, mais les gros bonus sont réservés
    // aux vraies difficultés. Le coefficient est calculé côté store pour
    // empêcher l'UI de fabriquer arbitrairement de l'XP.
    const difficultyMultiplier = { easy: 0.8, normal: 1, hard: 1.25, extreme: 1.5 }[difficulty] || 1;
    const reflectionBonus = (reflection || '').trim().length >= 20 ? 1.05 : 1;
    const streakBonus = Math.min(1.20, 1 + Math.max(0, (daily.streak || 0) - 1) * 0.02);
    const rawXp = Math.round(baseXp * difficultyMultiplier * reflectionBonus * streakBonus);
    const safeXp = Math.max(1, Math.min(rawXp, remainingDailyXp));
    ...
    if (previousDate === today) streak = Math.max(1, streak);
    else if (previousDate === yesterdayKey) streak += 1;
    else streak = 1;

    const nextProfile = { ...(this.state.developmentProfile || {}) };
    Object.entries(dimensions).forEach(([key, value]) => {
      if (typeof value !== 'number' || !Number.isFinite(value)) return;
      nextProfile[key] = Math.max(0, (nextProfile[key] || 0) + value);
    });
    nextProfile.actions = (nextProfile.actions || 0) + 1;

    const nowIso = now.toISOString();
    const evidence = {
      id: uid('dev'), challengeId: challengeId || null, type: type || 'action',
      label: cleanLabel, xp: safeXp, baseXp, multiplier: Number((safeXp / baseXp).toFixed(2)),
      difficulty, dimensions: { ...dimensions }, reflection: (reflection || '').trim(),
      date: today, createdAt: nowIso,
    };

    const completedIds = { ...(daily.completedIds || {}), [idKey]: nowIso };
    this.set({
      developmentProfile: nextProfile,
      developmentEvidence: [evidence, ...(this.state.developmentEvidence || [])],
      developmentChallengeState: {
        ...daily, dailyDate: today, dailyXp: (daily.dailyXp || 0) + safeXp,
        dailyActions: (daily.dailyActions || 0) + 1, streak, lastActionDate: today, completedIds,
      },
    });
    this.addXp(safeXp, 'development', cleanLabel);
    return { ok: true, evidence, streak, dailyXp: (daily.dailyXp || 0) + safeXp, dailyActions: (daily.dailyActions || 0) + 1 };
  }

  getDevelopmentChallengeStats() {
    const today = new Date().toISOString().slice(0, 10);
    const raw = this.state.developmentChallengeState || {};
    const daily = raw.dailyDate === today ? raw : { ...raw, dailyDate: today, dailyXp: 0, dailyActions: 0 };
    const evidence = this.listDevelopmentEvidence();
    return {
      dailyXp: daily.dailyXp || 0, dailyActions: daily.dailyActions || 0, streak: daily.streak || 0,
      maxDailyXp: 500, maxDailyActions: 5, evidenceCount: evidence.length,
      lastActionDate: daily.lastActionDate || null,
    };
  }

  listDevelopmentEvidence() {
    return (this.state.developmentEvidence || []).slice()
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  }
  ...
  getCoachContext() {
    ...
    return {
      user: { ...(snapshot.user || {}) },
      mood: { ...(snapshot.mood || {}) },
      emotions: (snapshot.emotions || []).slice(0, 30),
      behaviorCheckins: (snapshot.behaviorCheckins || []).slice(0, 50),
      developmentProfile: { ...(snapshot.developmentProfile || {}) },
      developmentEvidence: (snapshot.developmentEvidence || []).slice(0, 50),
      developmentChallengeState: { ...(snapshot.developmentChallengeState || {}) },
      activePawaModes: [...(snapshot.activePawaModes || [])],
      analytics: getPawaAnalytics(snapshot),
    };
  }

  getDevelopmentProfile() {
    return { ...(this.state.developmentProfile || {}) };
  }

  getActivePawaModes() {
    return Array.isArray(this.state.activePawaModes) ? [...this.state.activePawaModes] : [];
  }

  togglePawaMode(modeId) {
    if (!modeId) return this.getActivePawaModes();
    const current = this.getActivePawaModes();
    const next = current.includes(modeId)
      ? current.filter(id => id !== modeId)
      : [...current, modeId];
    this.set({ activePawaModes: next });
    return next;
  }

  // ============================================================
  // ACCOMPLISHMENTS — accomplissement du jour (texte libre), avec
  // confiance en soi / estime de soi / points calculés automatiquement
  // par l'app — pas de saisie manuelle de ces 3 valeurs.
  //
  // Barème volontairement simple et transparent (pas de "boîte noire") :
  //  - confidence (0-10) : longueur/détail du texte (un accomplissement
  //    décrit avec précision reflète plus de recul et donc plus de
  //    confiance dans ce qui a été fait) + petit bonus si un mot-clé
  //    d'effort/dépassement est présent ("difficile", "peur", "osé"...).
  //  - selfEsteem (0-10) : construit sur la régularité — le nombre de
  //    jours consécutifs (streak, jour du jour inclus) où au moins un
  //    accomplissement a été noté. Se sentir capable de recommencer
  //    chaque jour est ce qui nourrit l'estime de soi dans la durée,
  //    plus qu'un accomplissement isolé.
  //  - points : confidence + selfEsteem, cumulés dans le temps (total
  //    consultable via getAccomplishmentStats()).
  // ============================================================

  /** Mots-clés FR indiquant un effort/dépassement de soi (heuristique simple, non exhaustive). */
  static ACCOMPLISHMENT_EFFORT_WORDS = [
    'difficile', 'peur', 'osé', 'ose', 'malgré', 'échec', 'echouer', 'persévéré',
    'perseverer', 'dépassé', 'depasse', 'courage', 'stress', 'anxiété', 'anxiete',
    'première fois', 'premiere fois', 'jamais fait',
  ];

  _computeAccomplishmentConfidence(text) {
    const len = (text || '').trim().length;
    // 0-6 points selon la longueur (détail donné), plafonné à 6.
    let score = Math.min(6, Math.round(len / 20));
    const lower = (text || '').toLowerCase();
    if (Store.ACCOMPLISHMENT_EFFORT_WORDS.some((w) => lower.includes(w))) score += 3;
    if (len > 0) score += 1; // tout accomplissement noté vaut au moins un point de base
    return Math.min(10, Math.max(0, score));
  }

  _computeAccomplishmentStreakAt(dateStr, excludeId = null) {
    // Compte les jours consécutifs se terminant à dateStr (inclus) où au
    // moins un accomplissement existe déjà — utilisé pour dériver selfEsteem.
    const dates = new Set(
      this.state.accomplishments.filter((a) => a.id !== excludeId).map((a) => a.date)
    );
    dates.add(dateStr);
    let streak = 0;
  ...
  _computeAccomplishmentSelfEsteem(dateStr, excludeId = null) {
    const streak = this._computeAccomplishmentStreakAt(dateStr, excludeId);
    // 1 jour = 4, puis +1 par jour de série en plus, plafonné à 10.
    return Math.min(10, 3 + streak);
  }

  /** fields: { text, date (YYYY-MM-DD, défaut aujourd'hui) } — confidence,
   * selfEsteem et points sont TOUJOURS calculés par l'app, jamais saisis. */
  addAccomplishment(fields) {
    const id = uid('ac');
    const now = new Date().toISOString();
    const date = fields.date || now.slice(0, 10);
    const text = fields.text?.trim() || '';
    const confidence = this._computeAccomplishmentConfidence(text);
    const selfEsteem = this._computeAccomplishmentSelfEsteem(date);
    const accomplishment = {
      ...
      confidence,
      selfEsteem,
      points: confidence + selfEsteem,
      createdAt: now,
    };
    this.set({ accomplishments: [accomplishment, ...this.state.accomplishments] });
    return id;
  }

  deleteAccomplishment(id) {
    this.set({ accomplishments: this.state.accomplishments.filter((a) => a.id !== id) });
  }

  listAccomplishments() {
    return this.state.accomplishments.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  }

  /** Stats agrégées pour l'affichage (total points, streak actuel, moyennes). */
  getAccomplishmentStats() {
    const list = this.state.accomplishments;
    const today = new Date().toISOString().slice(0, 10);
    const totalPoints = list.reduce((sum, a) => sum + (a.points || 0), 0);
    const avg = (key) => (list.length ? Math.round((list.reduce((s, a) => s + (a[key] || 0), 0) / list.length) * 10) / 10 : 0);
    return {
      totalPoints,
      currentStreak: this._computeAccomplishmentStreakAt(today),
      avgConfidence: avg('confidence'),
      avgSelfEsteem: avg('selfEsteem'),
      count: list.length,
    };
  }

  // ============================================================
  // EVIDENCE ENGINE (fondations) — génération automatique d'un
  // Accomplishment à partir d'une action RÉELLEMENT complétée (objectif,
  // projet, habitude, mission), et crédit d'XP aux compétences détectées.
  //
  // Principe : ACTION → ACCOMPLISHMENT → EVIDENCE → DEVELOPMENT. Cette
  // méthode ne remplace ni ne modifie addXp() — elle est appelée EN PLUS,
  // aux mêmes points de complétion déjà existants (toggleGoalStep,
  // toggleProjectTask, setProjectStatus, toggleHabitToday, toggleMission),
  // pour ne jamais dupliquer le système XP global déjà en place.
  //
  // Anti-doublon : sourceId (id du goal/projet/habitude/mission) est stocké
  // sur l'accomplishment auto ; on ne recrée jamais de preuve pour le même
  // sourceId déjà couvert par un accomplishment non supprimé.
  // ============================================================

  /** type: 'goal_completed' | 'project_completed' | 'habit_completed' | 'task_completed' */
  /**
   * `development`, optionnel : { skills: string[] (ids data/developmentSkills.js),
   * primarySkill: string|null }. Fourni quand l'utilisateur a explicitement
   * coché des compétences dans le tiroir "Qu'est-ce que cette tâche
   * développe ?" (voir Tasks Agenda). Absent (ancienne tâche, ou objectif/
   * projet/habitude qui n'a pas encore cette UI) → comportement strictement
   * inchangé : détection automatique par mots-clés sur le libellé, comme
   * avant. Aucun deuxième système XP : dans les deux cas on retombe sur
   * creditSkillXp/splitXpAcrossSkills.
   */
  recordEvidenceFromCompletion({ type, sourceId, label, xpAmount, development }) {
    if (!sourceId || !label) return null;
    const alreadyRecorded = this.state.accomplishments.some(
      (a) => a.auto && a.sourceId === sourceId && a.type === type
    );
    if (alreadyRecorded) return null;

    const explicitSkillIds = Array.isArray(development?.skills) ? development.skills.filter(Boolean) : [];
    const resolvedExplicit = explicitSkillIds.length
      ? resolveDevelopmentSkills(explicitSkillIds, development?.primarySkill || null)
      : [];

    // Priorité aux compétences choisies à la main par l'utilisateur ; le
    // dictionnaire par mots-clés ne sert que de repli quand rien n'a été
    // sélectionné (ou pour les tâches créées avant cette fonctionnalité).
    const sourceKey = { goal_completed: 'objectif', project_completed: 'projet', habit_completed: 'habitude', task_completed: 'mission' }[type] || 'autre';
    const detectedSkills = resolvedExplicit.length ? resolvedExplicit : detectSkillsForCompletion(sourceKey, label);
    const xpShares = splitXpAcrossSkills(Math.max(1, xpAmount || 10), detectedSkills.length);

    const id = uid('ac');
    const now = new Date().toISOString();
    const difficulty = classifyDifficulty(xpAmount || 10);
    const accomplishment = {
      id,
      auto: true, // distingue des accomplissements texte-libre saisis à la main
      type,
      title: label,
      date: now.slice(0, 10),
      sourceId,
      difficulty, // minor | meaningful | major | milestone
      skills: detectedSkills.map((s) => s.name),
      // confidence/selfEsteem calculés comme pour un accomplissement manuel,
      // à partir du libellé — garde une échelle unique et comparable entre
      // preuves manuelles et automatiques.
      confidence: this._computeAccomplishmentConfidence(label),
      selfEsteem: this._computeAccomplishmentSelfEsteem(now.slice(0, 10)),
      createdAt: now,
    };
    accomplishment.points = accomplishment.confidence + accomplishment.selfEsteem;
    this.set({ accomplishments: [accomplishment, ...this.state.accomplishments] });

    detectedSkills.forEach((skill, i) => {
      this.creditSkillXp(skill, xpShares[i] || 1, id);
    });

    // Capability evidence : une action réellement complétée est par nature
    // une preuve de capacité ("I CAN"). On l'enregistre systématiquement
    // dans le journal Confidence, séparé de l'estime de soi (voir plus bas).
    this.recordConfidenceEvidence({ type, sourceId, label });

    // Self-Esteem : les compétences seules ne suffisent JAMAIS à en générer
    // (règle §7) — seules celles explicitement associées à une dimension
    // Self-Esteem dans le catalogue (data/developmentSkills.js, champ
    // `selfEsteem`) déclenchent une preuve, et uniquement quand l'utilisateur
    // a réellement coché ce skill pour cette tâche. Une seule preuve par
    // dimension et par tâche (anti-doublon déjà géré par recordSelfEsteemEvidence
    // via sourceId+type).
    resolvedExplicit
      .filter((s) => s.selfEsteem)
      .forEach((s) => {
        this.recordSelfEsteemEvidence({ type: s.selfEsteem, sourceId, label });
      });
  ...
  /**
   * Ajoute une PREUVE (note) sur une compétence déjà existante — ce n'est
   * PAS un doublon de creditSkillXp : c'est le point d'entrée pour le
   * formulaire "+ Ajouter une compétence" quand l'utilisateur choisit une
   * compétence déjà présente dans sa liste (au lieu d'en créer une nouvelle
   * — voir Skills.js, autocomplétion sur `skillId`). Réutilise exactement le
   * même moteur que les tâches (creditSkillXp/xpToLevel) et le même journal
   * Confidence (type 'skill_learned', déjà prévu dans confidenceTriggers.js)
   * — aucun deuxième système. `note` est obligatoire : sans preuve concrète,
   * pas de gain (voir PHILOSOPHIE §13, NO EMPTY PRAISE).
   * `xpAmount` : même valeur par défaut (10) qu'une tâche classique
   * complétée sans indication de difficulté — cohérence avec le reste de
   * l'app plutôt qu'un barème séparé pour les preuves manuelles.
   * Renvoie l'id de l'accomplissement créé, ou null si `note` est vide.
   */
  addSkillEvidence(skillId, note, xpAmount = 10) {
    const skill = this.getSkill(skillId);
    const trimmedNote = (note || '').trim();
    if (!skill || !trimmedNote) return null;
    ...
    const accomplishment = {
      ...
      title: trimmedNote,
      date: now.slice(0, 10),
      sourceId: id, // chaque preuve manuelle est son propre événement (pas de source partagée à dédupliquer, contrairement à une tâche) — l'utilisateur doit pouvoir noter plusieurs progrès sur la même compétence
      difficulty: classifyDifficulty(xpAmount),
      skills: [skill.name],
      confidence: this._computeAccomplishmentConfidence(trimmedNote),
      selfEsteem: this._computeAccomplishmentSelfEsteem(now.slice(0, 10)),
      createdAt: now,
    };
    accomplishment.points = accomplishment.confidence + accomplishment.selfEsteem;
    ...
    this.creditSkillXp({ name: skill.name, category: skill.category }, xpAmount, id);
    this.recordConfidenceEvidence({ type: 'skill_learned', sourceId: id, label: trimmedNote });

    return id;
  }

  /**
   * Retrouve l'accomplissement auto qui a généré une preuve Confidence/
   * Self-Esteem donnée, pour afficher la transparence "Because you
   * completed X → developed Y" (§12 du cahier des charges). `type` est le
   * type de l'ACCOMPLISSEMENT (ex: 'task_completed'), pas celui de la
   * preuve Self-Esteem (ex: 'courage') — les deux diffèrent quand la preuve
   * vient d'un skill développement (voir recordEvidenceFromCompletion). Si
   * `type` n'est pas fourni, on matche uniquement sur sourceId (suffisant
   * en pratique : sourceId est l'id de la tâche/objectif/projet source,
   * déjà unique). Renvoie null si rien ne correspond (preuve antérieure à
   * ce champ, accomplissement supprimé, etc.) — jamais d'exception.
   */
  getAccomplishmentForEvidence(sourceId, type = null) {
    if (!sourceId) return null;
    return this.state.accomplishments.find(
      (a) => a.auto && a.sourceId === sourceId && (type == null || a.type === type)
    ) || null;
  }

  // ============================================================
  // CONFIDENCE ENGINE — "I CAN DO IT."
  // Journal de preuves de CAPACITÉ, distinct de l'estime de soi. Alimenté
  // automatiquement par recordEvidenceFromCompletion() (objectifs, projets,
  // habitudes, tâches Agenda) et par toute résolution de problème réelle
  // (voir toggleProblemSolved plus haut). Ne dépend jamais d'un score
  // arbitraire seul : la stat sert à afficher les PREUVES concrètes.
  // ============================================================

  /** type: 'goal_completed' | 'project_completed' | 'habit_completed' |
   *  'task_completed' | 'problem_solved' | 'skill_learned' */
  recordConfidenceEvidence({ type, sourceId, label }) {
    if (!label) return null;
    if (sourceId) {
      const alreadyRecorded = this.state.confidenceEvidence.some(
        (e) => e.sourceId === sourceId && e.type === type
      );
      if (alreadyRecorded) return null;
    }
    const id = uid('conf');
    const now = new Date().toISOString();
    const evidence = { id, type, label, sourceId: sourceId || null, date: now.slice(0, 10), createdAt: now };
    this.set({ confidenceEvidence: [evidence, ...this.state.confidenceEvidence] });
    return id;
  }

  listConfidenceEvidence() {
    return this.state.confidenceEvidence.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  }

  /** Stats agrégées par type — sert à afficher des preuves ("34 problèmes résolus")
   * plutôt qu'un score arbitraire seul (voir PHILOSOPHIE §7). */
  getConfidenceStats() {
    const list = this.state.confidenceEvidence;
    const countByType = {};
    list.forEach((e) => { countByType[e.type] = (countByType[e.type] || 0) + 1; });
    return { total: list.length, countByType, recent: list.slice(0, 20) };
  }

  // ============================================================
  // SELF-ESTEEM ENGINE — "I HAVE VALUE."
  // Distinct de Confidence : pas alimenté par la performance seule, mais
  // par promesses tenues, courage, responsabilité, retour après échec,
  // contribution aux autres, respect de soi, croissance personnelle.
  // Certaines actions (voir recordEvidenceFromCompletion) alimentent aussi
  // Confidence ET Self-Esteem quand elles témoignent des deux à la fois
  // (ex : une habitude reprise après une pause alimente aussi 'comeback').
  // ============================================================

  /** type: 'promise_kept' | 'courage' | 'responsibility' | 'comeback' |
   *  'contribution' | 'growth' | 'self_respect' */
  recordSelfEsteemEvidence({ type, sourceId, label }) {
    if (!label) return null;
    if (sourceId) {
      const alreadyRecorded = this.state.selfEsteemEvidence.some(
        (e) => e.sourceId === sourceId && e.type === type
      );
      if (alreadyRecorded) return null;
    }
    ...
    const now = new Date().toISOString();
    const evidence = { id, type, label, sourceId: sourceId || null, date: now.slice(0, 10), createdAt: now };
    this.set({ selfEsteemEvidence: [evidence, ...this.state.selfEsteemEvidence] });
    return id;
  }

  listSelfEsteemEvidence() {
    return this.state.selfEsteemEvidence.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  }

  getSelfEsteemStats() {
    const list = this.state.selfEsteemEvidence;
    const countByType = {};
    list.forEach((e) => { countByType[e.type] = (countByType[e.type] || 0) + 1; });
    return { total: list.length, countByType, recent: list.slice(0, 20) };
  }
  ...
  // ============================================================
  // BOOST ME — sélection d'une preuve réelle (Confidence ou Self-Esteem)
  // adaptée à la situation courante, pour accompagner (jamais remplacer)
  // le contenu déjà choisi par getBoostSuggestion(). Ne renvoie jamais une
  // citation générique : uniquement des preuves construites à partir de
  // l'historique réel, ou null si l'utilisateur n'a encore aucune preuve.
  // ============================================================

  getBoostMeEvidence() {
    const confidence = this.listConfidenceEvidence().slice(0, 3);
    const selfEsteem = this.listSelfEsteemEvidence().slice(0, 3);
    if (!confidence.length && !selfEsteem.length) return null;
    // Alterne le type de trigger mis en avant pour ne pas toujours pousser
    // Confidence au détriment de Self-Esteem (ou l'inverse).
    const useConfidence = confidence.length && (!selfEsteem.length || Math.random() < 0.5);
    const chosen = useConfidence ? confidence[0] : selfEsteem[0];
    const trigger = useConfidence ? pickConfidenceTrigger(chosen.type) : pickSelfEsteemTrigger(chosen.type);
    const proofs = (useConfidence ? confidence : selfEsteem).map((e) => e.label);
    return { kind: useConfidence ? 'confidence' : 'selfEsteem', headline: trigger.headline, body: trigger.body, proofs };
  }

  // ============================================================
  // DEVELOPMENT SUMMARY — pour "Remind Me Who I Am" (voir AbandonMode et
  // getAbandonModeData ci-dessous, qui reste la source pour cet écran).
  // Rassemble un résumé texte dynamique basé uniquement sur des données
  // réelles (accomplissements, compétences, confiance, estime de soi).
  // ============================================================

  getDevelopmentSummary() {
    const accomplishments = this.state.accomplishments.length;
    const skillsLearned = this.state.skills.filter((s) => (s.xp || 0) > 0).length;
    const problemsSolved = this.state.confidenceEvidence.filter((e) => e.type === 'problem_solved').length;
    const comebacks = this.state.selfEsteemEvidence.filter((e) => e.type === 'comeback').length;
    return {
      accomplishments,
      skillsLearned,
      problemsSolved,
      comebacks,
      confidenceTotal: this.state.confidenceEvidence.length,
      selfEsteemTotal: this.state.selfEsteemEvidence.length,
    };
  }
  ...
  toggleProjectTask(projectId, taskId) {
    ...
    if (isNowComplete && !wasCompleted) {
      this.addXp(30, 'projet', project.name);
      this.recordEvidenceFromCompletion({ type: 'project_completed', sourceId: projectId, label: project.name, xpAmount: 30 });
    } else if (!isNowComplete && wasCompleted) {
      this.addXp(-30, 'projet', project.name);
    }
  }
  ...
  toggleHabitToday(id) {
    ...
    if (!doneToday) {
      this.addXp(8, 'habitude', habit.name);
      // sourceId composite (habitude + jour) : une habitude est quotidienne,
      // donc l'anti-doublon doit s'appliquer par jour, pas une seule fois
      // pour la vie de l'habitude — sinon plus aucune preuve après le 1er coche.
      this.recordEvidenceFromCompletion({
  ...
  checkDueNotifications() {
    ...
    Object.entries(this.state.notificationPrefs).forEach(([typeId, pref]) => {
      ...
      // Comme scheduled_payment_reminder : n'a de sens que si la leçon du jour existe
      // et n'a pas déjà été marquée comme lue (voir markDailyLessonRead()).
      if (typeId === 'daily_lesson' && (!this.state.dailyLesson || this.state.dailyLessonRead)) return;
  ...
  _notificationLabel(typeId) {
    ...
    const labels = {
      ...
      daily_lesson: this.state.dailyLesson
        ? `Ta leçon du jour t'attend : ${this.state.dailyLesson.title} (${this.state.dailyLesson.theme}).`
        : 'Une nouvelle leçon du jour est disponible.',
    };
