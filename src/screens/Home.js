...
/**
 * Carte "Leçon du jour" : thème, titre, corps en plusieurs paragraphes,
 * exercice pratique, et un bouton pour la marquer comme lue (crédite l'XP
 * une seule fois par jour — voir store.markDailyLessonRead()).
 * Tant que non lue : bordure et badge en couleur ember (accent chaud de
 * l'app) pour attirer l'œil, comme un vrai "reminder" visuel sur Home,
 * sans dépendre uniquement de la notification native. Une fois lue,
 * la carte repasse en style neutre standard.
 */
function lessonCard(lesson, read) {
  if (!lesson) return '';
  return `
    <section class="card" aria-label="Leçon du jour" style="${read ? '' : 'border: 1.5px solid var(--ember-500); box-shadow: 0 0 0 1px var(--ember-500) inset;'}">
      <div class="card__label" style="display:flex; align-items:center; justify-content:space-between">
        <span>Leçon du jour · ${lesson.theme}</span>
        ${read ? '' : `<span class="chip" style="background: var(--ember-500); color: #fff; font-size: var(--fs-xs)">Nouveau</span>`}
      </div>
      <h3 style="margin: var(--sp-2) 0; font-size: var(--fs-lg)">${lesson.title}</h3>
      ${lesson.body.map((p) => `<p class="detail-desc" style="margin-bottom: var(--sp-3)">${p}</p>`).join('')}
      <div class="card" style="background: var(--bg-surface-raised); margin-top: var(--sp-2)">
        <div class="card__label">Exercice</div>
        <p class="detail-desc">${lesson.exercise}</p>
      </div>
      <button class="chip" id="lesson-read-btn" style="margin-top: var(--sp-3); ${read ? '' : 'background: var(--ember-500); color: #fff; border-color: var(--ember-500);'}" ${read ? 'disabled' : ''}>
        ${read ? `${icons.check}Leçon lue aujourd'hui` : `${icons.check}Marquer comme lue (+${lesson.xp} XP)`}
      </button>
    </section>`;
}
 ...
 * Renvoie un message "pas encore de données" si la série est vide ou n'a
 * qu'un seul point (une courbe à un seul point n'a rien à montrer).
 */
...
export function Home() {
  ...
  const charts = store.getHomeChartsData();
  ...
  screen.innerHTML = `
    ...
    <section class="boost-dial-wrap">
      ...
      <div class="detail-tags-row" style="justify-content:center; margin-top: var(--sp-2)">
        <button class="chip" id="go-coach-btn">${icons.sparkles.replace('<svg ', '<svg style="width:16px;height:16px;stroke:currentColor;stroke-width:2;flex-shrink:0;margin-right:4px;vertical-align:-3px" ')}Parler à Mon Coach →</button>
        <button class="chip" id="go-dev-btn">${icons.compass.replace('<svg ', '<svg style="width:16px;height:16px;stroke:currentColor;stroke-width:2;flex-shrink:0;margin-right:4px;vertical-align:-3px" ')}Développement →</button>
        <button class="chip" id="go-abandon-btn">${icons.heart.replace('<svg ', '<svg style="width:16px;height:16px;stroke:currentColor;stroke-width:2;flex-shrink:0;margin-right:4px;vertical-align:-3px" ')}Je veux abandonner</button>
      </div>
    </section>
    ...
    <section class="card" aria-label="Missions du jour">
      ...
      <button class="chip" id="go-reports-btn" style="margin-top: var(--sp-3)">${icons.notes.replace('<svg ', '<svg style="width:16px;height:16px;stroke:currentColor;stroke-width:2;flex-shrink:0;margin-right:4px;vertical-align:-3px" ')}Rapport du jour →</button>
    </section>

    ${lessonCard(state.dailyLesson, state.dailyLessonRead)}
    ...
    <div class="card__label" style="margin-top: var(--sp-5)">Tendances (14 derniers jours)</div>
    <section class="card" aria-label="Courbes de tendance">
      <div class="card__label">XP cumulé</div>
      ${sparkline(charts.xpSeries, 'var(--steel-400)', "Pas encore assez de données — reviens dans quelques jours.")}

      <div class="card__label" style="margin-top: var(--sp-4)">Énergie</div>
      ${sparkline(charts.moodSeries, 'var(--success-500)', "Fais un check-in émotionnel pour commencer à suivre ton énergie.")}

      <div class="card__label" style="margin-top: var(--sp-4)">Progression des objectifs actifs</div>
      ${sparkline(charts.goalSeries, 'var(--ember-500)', "Avance sur un objectif pour voir sa progression ici.")}
    </section>
  ...
  screen.querySelector('#lesson-read-btn')?.addEventListener('click', () => {
    store.markDailyLessonRead();
    el.replaceWith(Home());
  });
