  ...
  row.innerHTML = `
    <div class="goal-card__top">
      <div class="goal-card__title-row">
        ${(icons[type.icon] || icons.bolt).replace('<svg ', '<svg style="width:18px;height:18px;stroke:currentColor;stroke-width:2;flex-shrink:0" ')}
    ...
    <details class="type-row-details" style="margin-top:var(--sp-1)">
      <summary style="cursor:pointer; list-style:none; font-size:var(--fs-sm); color:var(--text-secondary)">Détails</summary>
      <p class="detail-desc" style="margin-top:var(--sp-1)">${type.description}</p>
    </details>
    ${pref.enabled ? `
    <div class="form-group" style="margin-top:var(--sp-2)">
      <input class="form-input" type="time" value="${pref.time}" data-time="${type.id}" style="padding:var(--sp-2); height:auto;" />
    </div>
    <div class="goal-card__meta-row" style="margin-top:var(--sp-2); gap:var(--sp-4)">
      <button type="button" class="category-chip ${pref.sound !== false ? 'is-active' : ''}" data-sound-toggle="${type.id}">🔊 Son</button>
      <button type="button" class="category-chip ${pref.vibrate !== false ? 'is-active' : ''}" data-vibrate-toggle="${type.id}">📳 Vibration</button>
    </div>` : ''}
  `;
  return row;
}
...
export function Profile() {
  ...
  screen.querySelectorAll('[data-sound-toggle]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const typeId = btn.dataset.soundToggle;
      const current = store.getNotificationPrefs()[typeId];
      const newValue = !(current.sound !== false);
      store.setNotificationPref(typeId, { sound: newValue });
      if (newValue) store.testSound();
      rerender();
    });
  });

  screen.querySelectorAll('[data-vibrate-toggle]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const typeId = btn.dataset.vibrateToggle;
      const current = store.getNotificationPrefs()[typeId];
      const newValue = !(current.vibrate !== false);
      store.setNotificationPref(typeId, { vibrate: newValue });
      if (newValue) store.testVibrate();
      rerender();
    });
  });
