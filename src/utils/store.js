...
const defaultState = {
  ...
  agendaTasks: [], // AgendaTask[] — { id, date (YYYY-MM-DD), hour (0-23), duration (heures, >=1), title, note, done, doneAt, createdAt }
  agendaXp: {
    xp: 0,
    totalEarned: 0,
  },
...
class Store {
  ...
  /** Crée une tâche planifiée. hour = 0-23. duration = nombre d'heures que la tâche occupe (>=1). */
  addAgendaTask({ date, hour, duration = 1, title, note = '' }) {
    const task = {
      id: uid('agenda'),
      date,
      hour: Number(hour),
