  ...
  screen.innerHTML = `
    ...
    <section class="card">
      <div class="detail-desc" style="display:flex; flex-direction:column; gap:var(--sp-2)">
        <div style="display:flex; gap:var(--sp-2); align-items:flex-start">${icons.shieldCheck}<span>Secret n'est jamais envoyé à l'IA, automatiquement ou non.</span></div>
        <div style="display:flex; gap:var(--sp-2); align-items:flex-start">${icons.shieldCheck}<span>Aucune clé API n'est stockée ni visible dans l'application.</span></div>
        <div style="display:flex; gap:var(--sp-2); align-items:flex-start">${icons.shieldCheck}<span>Tu peux exporter, importer ou supprimer toutes tes données à tout moment.</span></div>
        <div style="display:flex; gap:var(--sp-2); align-items:flex-start">${icons.shieldCheck}<span>Tu peux désactiver l'IA complètement dans Profil.</span></div>
      </div>
    </section>
